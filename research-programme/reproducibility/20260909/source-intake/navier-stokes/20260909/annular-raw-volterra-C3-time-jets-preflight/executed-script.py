...
def run():
    ...
    def direct_rhs(engine, offset, state):
        ...
        complete, diagnostics = complete_without_scalar_projection(raw, current["old_gradient"], engine.spacing)
    ...
    try:
        ...
        folder = intake / ("annular-coupled-current-analytic-third-corner" if arguments.mode == "preflight" else "annular-coupled-current-third-corner-raw-volterra")
        ...
        for case_name in ["canonical", "nonlinear_modulated"]:
            ...
            for entry in owner["runs"]:
                ...
                engine = RawVolterraTimeJets(evaluator, radii, entry["time"])
                ...
                side_change = None
                if entry["time"] == 0.1:
                    other = RawVolterraTimeJets(evaluator, radii, entry["time"], side="right")
