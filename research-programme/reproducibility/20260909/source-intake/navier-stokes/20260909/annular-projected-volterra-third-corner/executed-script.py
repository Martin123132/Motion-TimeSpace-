...
def run():
    ...
    from annular_volterra_source_completion_20260909 import derivative_moments
    from annular_projected_volterra_source_20260909 import complete_projected_volterra

    parser = argparse.ArgumentParser()
    parser.add_argument("--tag", default="annular-projected-volterra-third-corner")
    parser.add_argument("--max-grid", type=int, choices=[128, 256, 512], default=512)
    parser.add_argument("--cfl", type=float, default=0.15)
    parser.add_argument("--dissipation", type=float, choices=[0, 1 / 64], default=0)
    parser.add_argument("--source-completion", choices=["projected", "projected_volterra", "discrete_chain"], default="projected_volterra")
    ...
    if arguments.source_completion == "projected_volterra":
        report["parameters"]["mass_completion"] = "Backward cell-exponential Volterra solve: original projected q/lapse source law retained everywhere; only mass source changed, outer mass source zero, nonzero all-row source residual explicitly bounded. Physical/field accuracy gates unchanged; exact source identity is NOT claimed."
    ...
    try:
        ...
        if arguments.source_completion == "projected_volterra":
            ...
            path = intake / "annular-projected-volterra-time-jets-preflight/status.json"
            hybrid_owner = json.loads(path.read_text())
            own(path)
            verify("hybrid_operator_preflight_complete", hybrid_owner["state"] == "complete" and hybrid_owner["passed"] == hybrid_owner["total"] and path.with_name("COMPLETE").exists())
            verify("hybrid_preflight_inputs_unchanged", all(own(root / name) == digest for name, digest in hybrid_owner["inputs"].items()))
        for name in ["annular_volterra_source_completion_20260909.py", "annular_projected_volterra_source_20260909.py", "annular_projected_volterra_time_jets_20260909.py"]:
        ...
        for case_name in ["canonical", "nonlinear_modulated"]:
            ...
            for intervals in grids:
                ...
                def numerical_source(current, values):
                    ...
                    if arguments.source_completion == "projected_volterra":
                        complete, source_diagnostics = complete_projected_volterra(raw[0], current["old_gradient"], band, outer, weights * radii**2 * current["alpha"], spacing, moments=source_moments)
                ...
                for time_refinement in ([1, 2] if intervals == grids[-1] else [1]):
                    ...
                    for segment in range(3):
                        ...
                        if arguments.source_completion == "projected_volterra":
                            ...
                            projected_reference = project_interior_scalar_and_complete(diagnostics["raw_coordinate_numerical_source"][None], current["old_gradient"], band, outer, weights * radii**2 * current["alpha"])[0]
                            verify(output_tag + "_projected_scalar_lapse_source_law_unchanged", numerical.array_equal(diagnostics["coordinate_numerical_source"][[0, 1, 3]], projected_reference[[0, 1, 3]]))
                        else:
