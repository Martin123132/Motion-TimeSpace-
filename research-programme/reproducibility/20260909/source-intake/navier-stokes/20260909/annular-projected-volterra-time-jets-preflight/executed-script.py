...
def run():
    ...
    report = {"state": "running", "started_utc": datetime.now(timezone.utc).isoformat(), "script_sha256": hashlib.sha256(source).hexdigest(), "inputs": {}, "checks": [], "runs": [], "valid_for_physics_claim": False, "scope": "Mass-only Volterra replacement with original projected scalar/lapse source law. Differentiate the actual cell-exponential recurrence through time degree2, then actual RHS through derivative3. Preflight uses old saved states only as matched-state probes; trajectory mode replays new evolution. Nonzero source Noether residual retained, not exact source identity or physical claim."}
    ...
    try:
        ...
        verify("ordinary_evolution_owner_passed", owner["state"] == "complete" and owner["passed"] == owner["total"] and (folder / "COMPLETE").exists())
