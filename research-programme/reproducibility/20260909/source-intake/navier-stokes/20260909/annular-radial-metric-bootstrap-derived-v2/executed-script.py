...
def run():
    ...
    destination = intake / 'annular-radial-metric-bootstrap-derived-v2'
    ...
    for path in [gate_path, gate_path.parent / 'COMPLETE', gate_path.parent / 'executed-script.py', Path(__file__).resolve()]:
        own(path)
    for name in ['executed-script.py', 'FAILED.txt']:
        own(intake / 'annular-radial-metric-bootstrap-derived' / name)
