...
def run():
    ...
    parser.add_argument("--tag", default="annular-compatible-current-restoring-smoke")
    parser.add_argument("--max-grid", type=int, choices=[128], default=128)
    parser.add_argument("--cfl", type=float, default=0.15)
    parser.add_argument("--dissipation", type=float, choices=[0], default=0)
    parser.add_argument("--source-completion", choices=["projected", "compatible_volterra"], default="compatible_volterra")
    parser.add_argument("--initial-kind", choices=["legacy", "second_corner", "third_corner"], default="third_corner")
    ...
    report["parameters"]["compatible_restoring_term"] = arguments.source_completion == "compatible_volterra"
    report["parameters"]["timestep_control"] = "Ordinary CFL plus minimum qualified full-coupled frozen-matrix cap. Same caps for baseline; no all-time/continuum stability claim."
    if arguments.source_completion == "compatible_volterra":
        report["parameters"]["mass_completion"] = "Backward cell-exponential Volterra solve: whole raw SAT plus derived compatible Gram restoring source completed, without scalar projection, outer mass source zero, nonzero all-row source residual explicitly bounded. Physical/field accuracy gates unchanged; exact source identity is NOT claimed."
    ...
    save()
    try:
        path = intake / "annular-compatible-current-timestep-qualified/status.json"
        timestep_owner = json.loads(path.read_text())
        own(path)
        verify("coupled_timestep_qualification_complete", timestep_owner["state"] == "complete" and timestep_owner["passed"] == timestep_owner["total"] and path.with_name("COMPLETE").exists())
        verify("timestep_inputs_unchanged", all(own(root / name) == digest for name, digest in timestep_owner["inputs"].items()))
        verify("qualified_smoke_grids", grids == timestep_owner["grids"] and arguments.initial_kind == "third_corner")
        timestep_caps = {(row["case"], row["intervals"]): row["dt_cap"] for row in timestep_owner["dt_caps"]}
        for name in ["annular_compatible_current_restoring_20260909.py", "annular_compatible_current_time_jets_20260909.py", "annular_compatible_current_rhs_20260909.py"]:
        ...
        for case_name in ["canonical", "nonlinear_modulated"]:
            ...
            for intervals in grids:
                ...
                if arguments.source_completion == "compatible_volterra":
                    verify(label + "_positive_compatible_coefficient", numerical.min(cache["c"]) > 0)
                ...
                def numerical_source(current, values):
                    ...
                    if arguments.source_completion == "compatible_volterra":
                        raw[0, 1] += restoring_coordinate_source(values[0], radii, current, weights, spacing)
                ...
                for time_refinement in ([1, 2] if intervals == grids[-1] else [1]):
                    ...
                    per_segment = max(math.ceil(0.1 * maximum_speed * 1.05 / (arguments.cfl * spacing)), math.ceil(0.1 / timestep_caps[case_name, intervals])) * time_refinement
                    timestep = 0.1 / per_segment
                    verify(tag + "_qualified_timestep_cap", timestep <= timestep_caps[case_name, intervals] * (1 + 1e-12), {"dt": timestep, "cap": timestep_caps[case_name, intervals]})
                    values = lifted.copy()
