...
def run():
    ...
    destination = intake / 'annular-local-Ward-identity-derived-factored'
    ...
    try:
        ...
        verify('interrupted_simplifier_source_preserved', interrupted['state'] == 'interrupted' and own(interrupted_path.parent / 'executed-script.py') == interrupted['inputs'][str(Path(__file__).relative_to(root))])
