...
def run():
    ...
    destination = intake / 'annular-local-Ward-identity-derived-validated'
    ...
    try:
        ...
        interrupted_path = intake / 'annular-local-Ward-identity-derived/status.json'
        own(interrupted_path)
        interrupted = json.loads(interrupted_path.read_text(encoding='utf-8-sig'))
        interrupted_script = interrupted_path.parent / 'executed-script.py'
        own(interrupted_script)
        verify('interrupted_simplifier_source_preserved', interrupted['state'] == 'interrupted' and 'symbolic.simplify(variation - generator * time_derivative - generator_t * density)' in interrupted_script.read_text())
        metadata_failure_path = intake / 'annular-local-Ward-identity-derived-factored/status.json'
        own(metadata_failure_path)
        own(metadata_failure_path.parent / 'executed-script.py')
        metadata_failure = json.loads(metadata_failure_path.read_text())
        verify('precalculation_metadata_failure_preserved', metadata_failure['state'] == 'failed' and 'KeyError' in metadata_failure['failure'])
        ...
        spatial_f = symbolic.symbols('F', positive=True)
        ...
        temporal_coefficient = symbolic.diff(density, lapse) * lapse + symbolic.diff(density, velocity) * velocity - density
        radial_coefficient = symbolic.diff(density, mass_r) * mass_time + symbolic.diff(density, gradient) * velocity - shift_derivative * lapse**2 * spatial_f
        verify('symbolic_ADM_matter_generator_time_coefficient', symbolic.cancel(temporal_coefficient) == 0)
        verify('symbolic_ADM_matter_generator_radial_coefficient', symbolic.cancel(radial_coefficient) == 0)
        ...
        verify('symbolic_Gram_spatial_generator_cancellation_a_w_q_equals_gamma_a_V', symbolic.cancel((symbolic.diff(coefficient, gradient) * velocity - lapse**2 * spatial_f * symbolic.diff(coefficient, shift)).subs(shift, 0)) == 0)
        verify('symbolic_Gram_temporal_density_weight_a_q_q_plus_a_N_N_equals_a', symbolic.cancel((symbolic.diff(coefficient, velocity) * velocity + symbolic.diff(coefficient, lapse) * lapse - coefficient).subs(shift, 0)) == 0)
