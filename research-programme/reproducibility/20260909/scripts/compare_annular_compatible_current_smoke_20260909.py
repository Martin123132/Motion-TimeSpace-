...
def run():
    ...
    destination = intake / "annular-compatible-current-smoke-comparison"
    destination.mkdir(exist_ok=False)
    source = Path(__file__).read_bytes()
    (destination / "executed-script.py").write_bytes(source)
    report = {"state": "running", "started_utc": datetime.now(timezone.utc).isoformat(), "script_sha256": hashlib.sha256(source).hexdigest(), "inputs": {}, "checks": [], "norms": [], "refinement": [], "source_parity": [], "comparisons": [], "valid_for_physics_claim": False, "scope": "Matched original-versus-raw-source-Volterra comparison. Same C3 data, background, physical boundary data, grids, timesteps and gates; candidate adds derived compatible restoring force and Volterra completion. Source/bulk parity is an instantaneous diagnostic, not a proof of sole causality. Polynomial H3/H4 norms and trace lower bounds are not unknown continuum bounds."}
    ...
    def load_owner(name, allow_field_failure=False):
        ...
        if allow_field_failure:
            ...
            report["candidate_field_evolution_passed"] = owner["state"] == "complete" and owner["passed"] == owner["total"]
            report["retained_field_failures"] = [check for check in owner["checks"] if not check["passed"]]
            verify(name + "_failure_not_promoted", (folder / "COMPLETE").exists() == report["candidate_field_evolution_passed"])
        else:
    ...
    try:
        own(Path(__file__))
        derivation_path = intake / "annular-compatible-current-timestep-qualified/status.json"
        own(derivation_path)
        derivation = json.loads(derivation_path.read_text())
        verify("source_projection_derivation_complete", derivation["state"] == "complete" and derivation["passed"] == derivation["total"] == 76 and derivation_path.with_name("COMPLETE").exists())
        ...
        reconstructions = {
            "original": load_owner("annular-compatible-smoke-normal-baseline"),
            "candidate": load_owner("annular-compatible-smoke-normal-candidate"),
        }
        evolutions = {
            "original": load_owner("annular-compatible-current-matched-baseline"),
            "candidate": load_owner("annular-compatible-current-restoring-smoke", allow_field_failure=True),
        }
        original_inputs, candidate_inputs = [evolutions[branch][1]["inputs"] for branch in ["original", "candidate"]]
        ...
        verify("same_frozen_C3_payloads", len(matched_initials) >= 3 and all(candidate_inputs.get(name) == original_inputs[name] for name in matched_initials))
        verify("same_evolution_parameters_except_mass_completion", all(evolutions["original"][1]["parameters"][key] == evolutions["candidate"][1]["parameters"][key] for key in ["N", "T", "outputs_T", "epsilon", "sigma", "cfl", "time_interpolation_intervals", "dissipation", "initial_kind", "constraint_addition"]))
        baseline_steps = {(row["case"], row["time"], row["intervals"], row["time_refinement"]): row["dt"] for row in evolutions["original"][1]["runs"]}
        verify("same_actual_timesteps_and_grids", len(baseline_steps) == 16 and all(row["dt"] == baseline_steps[row["case"], row["time"], row["intervals"], row["time_refinement"]] for row in evolutions["candidate"][1]["runs"]))
        verify("same_field_acceptance_gates", evolutions["original"][1]["gates"] == evolutions["candidate"][1]["gates"])
        stored = {}
        ...
        for branch, (folder, owner) in evolutions.items():
            for entry in owner["runs"]:
                ...
                cell_engine = cell_engines[cell_key]
                constraint_gradient = cell_engine.current["constraint_gradient"][0]
                constraint_gradient_time = cell_engine.current["constraint_gradient"][1]
                reference_constraint = cell_engine.current["J0"][0]
                reference_constraint_time = cell_engine.current["J0"][1]
                state = arrays["correction_current"]
                rhs = arrays["current_rhs"]
                sources = arrays["numerical_source"]
                bulk_rhs = rhs - sources
                full_constraint = reference_constraint + derivative(state[3], spacing) - numerical.sum(constraint_gradient * state, axis=0)
                full_rate = reference_constraint_time + derivative(rhs[3], spacing) - numerical.sum(constraint_gradient_time * state + constraint_gradient * rhs, axis=0)
                source_rate = derivative(sources[3], spacing) - numerical.sum(constraint_gradient * sources, axis=0)
                bulk_rate = reference_constraint_time + derivative(bulk_rhs[3], spacing) - numerical.sum(constraint_gradient_time * state + constraint_gradient * bulk_rhs, axis=0)
                verify(branch + "_" + entry["artifact"] + "_full_constraint_replay", maximum(full_constraint - arrays["full_mass_constraint"]) < 1e-8 * max(maximum(full_constraint), 1e-20) + 1e-22)
                verify(branch + "_" + entry["artifact"] + "_constraint_rate_source_budget", maximum(full_rate - source_rate - bulk_rate) < 1e-8 * max(maximum(full_rate), maximum(source_rate), maximum(bulk_rate), 1e-20) + 1e-22)
                row = {key: entry[key] for key in ["case", "time", "intervals"]}
                row.update(full_constraint_rate_max=maximum(full_rate), bulk_constraint_rate_max=maximum(bulk_rate), source_constraint_rate_max=maximum(source_rate), rate_budget_is_instantaneous_not_time_integrated=True)
            ...
            for time_value in [0.1, 0.3]:
                for method in ["poly7", "poly9"]:
                    ...
                    for branch in ["original", "candidate"]:
                        differences = [norms(stored[branch, case_name, time_value, intervals, method], 4 / intervals, coarse=stored[branch, case_name, time_value, intervals // 2, method]) for intervals in [64, 128]]
                ...
                original, candidate = [next(row for row in report["norms"] if row["branch"] == branch and row["case"] == case_name and row["time"] == time_value and row["intervals"] == 128 and row["method"] == "poly7") for branch in ["original", "candidate"]]
                field = {branch: next(row for row in evolutions[branch][1]["comparisons"] if row["case"] == case_name and row["time"] == time_value) for branch in ["original", "candidate"]}
                curvature = {branch: next(row for row in reconstructions[branch][1]["sensitivity"] if row["case"] == case_name and row["time"] == time_value) for branch in ["original", "candidate"]}
                report["comparisons"].append({"case": case_name, "time": time_value, "candidate_over_original_mass_H4_trace_floor": candidate["mass_H4_trace_lower_bound_all_cells"] / max(original["mass_H4_trace_lower_bound_all_cells"], 1e-30), "candidate_over_original_physical_norm": candidate["measured_physical_H3chi_H4mu_H4delta_norm"] / max(original["measured_physical_H3chi_H4mu_H4delta_norm"], 1e-30), "field": field, "curvature": curvature, "valid_for_physics_claim": False})
        ...
        report["all_candidate_curvature_sensitivity_gates_passed"] = reconstructions["candidate"][1]["all_curvature_sensitivity_gates_passed"]
        report["all_candidate_strong_mesh_differences_reduced"] = all(row["branches"]["candidate"]["physical_ratio"] >= 1.3 for row in report["refinement"])
