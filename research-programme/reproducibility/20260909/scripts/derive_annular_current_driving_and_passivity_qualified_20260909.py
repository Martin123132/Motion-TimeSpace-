...
def run():
    ...
    destination = intake / "annular-current-driving-and-passivity-qualified"
    ...
    try:
        own(Path(__file__))
        first_folder = intake / "annular-current-driving-and-passivity-obstruction"
        own(first_folder / "status.json")
        first_owner = json.loads((first_folder / "status.json").read_text())
        verify("first_diagnostic_preserved", first_owner["state"] == "complete" and own(first_folder / "executed-script.py") == first_owner["script_sha256"] and all(own(root / name) == digest for name, digest in first_owner["inputs"].items()))
        report["qualification"] = "Replaces a redundant characteristic-polynomial check with the explicit derived formula, adds its actual-coefficient replay and sharp rank-two source energy algebra. Initial diagnostic remains immutable; no field result or gate changed."
        ...
        characteristic = principal.charpoly()
        spectral = characteristic.gen
        verify("acoustic_characteristic_polynomial", symbolic.simplify(characteristic.as_expr() - (spectral**2 - 2 * mixed * spectral / alpha - wave / alpha)) == 0)
        boundary_amplitude, interior_amplitude, gain = symbolic.symbols("boundary_amplitude interior_amplitude gain", real=True)
        source_power = -gain * boundary_amplitude * interior_amplitude
        symmetric_source = symbolic.Matrix([[0, -gain / 2], [-gain / 2, 0]])
        amplitudes = symbolic.Matrix([interior_amplitude, boundary_amplitude])
        verify("rank_two_source_energy_form", symbolic.simplify((amplitudes.T * symmetric_source * amplitudes)[0] - source_power) == 0)
        verify("sharp_finite_source_work_bound", symbolic.simplify((gain * (interior_amplitude**2 + boundary_amplitude**2) / 2 - source_power) - gain * (interior_amplitude + boundary_amplitude)**2 / 2) == 0)
        ...
        for case_name in ["canonical", "nonlinear_modulated"]:
            ...
            for branch, folder_name in [("original", "annular-coupled-current-analytic-third-corner"), ("hybrid", "annular-projected-volterra-third-corner")]:
                ...
                for entry in owner["runs"]:
                    ...
                    actual_block = numerical.stack([current["q_gradient"][[1, 2]], current["flux_gradient"][[1, 2]]])
                    expected_block = numerical.stack([numerical.stack([current["B"], numerical.ones_like(radii)]), numerical.stack([current["P"] * current["Q"], current["B"]])]) / current["alpha"]
                    verify(label + "_actual_acoustic_principal_identity", maximum(actual_block - expected_block) < 1e-10 * max(maximum(expected_block), 1e-20))
