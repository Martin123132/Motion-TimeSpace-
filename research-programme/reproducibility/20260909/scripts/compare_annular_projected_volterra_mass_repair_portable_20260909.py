...
def run():
    ...
    destination = intake / "annular-projected-volterra-matched-comparison-portable"
    destination.mkdir(exist_ok=False)
    source = Path(__file__).read_bytes()
    (destination / "executed-script.py").write_bytes(source)
    report = {"state": "running", "started_utc": datetime.now(timezone.utc).isoformat(), "script_sha256": hashlib.sha256(source).hexdigest(), "inputs": {}, "checks": [], "norms": [], "refinement": [], "source_parity": [], "comparisons": [], "valid_for_physics_claim": False, "scope": "Matched original-versus-mass-only-Volterra comparison. Same C3 data, background, scalar/lapse source law and gates. Source/bulk parity is an instantaneous diagnostic, not a proof of sole causality. Polynomial H3/H4 norms and trace lower bounds are not unknown continuum bounds."}
    ...
    def load_owner(name, allow_field_failure=False):
        ...
        if allow_field_failure:
            ...
            report["hybrid_field_evolution_passed"] = owner["state"] == "complete" and owner["passed"] == owner["total"]
            report["retained_field_failures"] = [check for check in owner["checks"] if not check["passed"]]
            verify(name + "_failure_not_promoted", (folder / "COMPLETE").exists() == report["hybrid_field_evolution_passed"])
        else:
    ...
    try:
        own(Path(__file__))
        previous_folder = intake / "annular-projected-volterra-matched-comparison"
        own(previous_folder / "status.json")
        previous = json.loads((previous_folder / "status.json").read_text())
        verify("prior_path_filter_failure_preserved", previous["state"] == "failed" and [row["name"] for row in previous["checks"] if not row["passed"]] == ["same_frozen_C3_payloads"] and not (previous_folder / "COMPLETE").exists())
        verify("prior_comparison_snapshot_preserved", own(previous_folder / "executed-script.py") == previous["script_sha256"])
        verify("prior_comparison_inputs_unchanged", all(own(root / name) == digest for name, digest in previous["inputs"].items()))
        report["harness_correction"] = "Only normalize Windows path separators in initial-payload selection. No numerical gate or result changed; first failed harness report retained."
        ...
        reconstructions = {
            ...
            "hybrid": load_owner("annular-projected-volterra-normal-reconstruction"),
        }
        evolutions = {
            "original": load_owner("annular-coupled-current-analytic-third-corner"),
            "hybrid": load_owner("annular-projected-volterra-third-corner", allow_field_failure=True),
        }
        preflight_folder, preflight = load_owner("annular-projected-volterra-time-jets-preflight")
        original_inputs, hybrid_inputs = [evolutions[branch][1]["inputs"] for branch in ["original", "hybrid"]]
        ...
        verify("same_frozen_C3_payloads", len(matched_initials) >= 3 and all(hybrid_inputs.get(name) == original_inputs[name] for name in matched_initials))
        verify("same_evolution_parameters_except_mass_completion", all(evolutions["original"][1]["parameters"][key] == evolutions["hybrid"][1]["parameters"][key] for key in ["N", "T", "outputs_T", "epsilon", "sigma", "cfl", "time_interpolation_intervals", "dissipation", "initial_kind", "constraint_addition"]))
        verify("same_field_acceptance_gates", evolutions["original"][1]["gates"] == evolutions["hybrid"][1]["gates"])
        stored = {}
        ...
        for branch, (folder, owner) in evolutions.items():
            for entry in owner["runs"]:
                ...
                row.update(branch=branch, source_parity_RMS=rms(source_indicator), bulk_parity_RMS=rms(bulk_indicator), RHS_parity_RMS=rms(rhs_indicator), field_parity_RMS=rms(parity(arrays["correction_current"][3], spacing)), diagnostic_not_claim=True)
                if branch == "original":
                    ...
                    row["same_state_hybrid_mass_source_parity_RMS"] = rms(parity(replacement, spacing))
                    row["same_state_hybrid_counterfactual_RHS_parity_RMS"] = rms(parity(bulk + replacement, spacing))
        ...
        for case_name in ["canonical", "nonlinear_modulated"]:
            for time_value in [0.1, 0.3]:
                ...
                original, hybrid = [next(row for row in report["norms"] if row["branch"] == branch and row["case"] == case_name and row["time"] == time_value and row["intervals"] == 512 and row["method"] == "poly7") for branch in ["original", "hybrid"]]
                field = {branch: next(row for row in evolutions[branch][1]["comparisons"] if row["case"] == case_name and row["time"] == time_value) for branch in ["original", "hybrid"]}
                curvature = {branch: next(row for row in reconstructions[branch][1]["sensitivity"] if row["case"] == case_name and row["time"] == time_value) for branch in ["original", "hybrid"]}
                report["comparisons"].append({"case": case_name, "time": time_value, "hybrid_over_original_mass_H4_trace_floor": hybrid["mass_H4_trace_lower_bound_all_cells"] / max(original["mass_H4_trace_lower_bound_all_cells"], 1e-30), "hybrid_over_original_physical_norm": hybrid["measured_physical_H3chi_H4mu_H4delta_norm"] / max(original["measured_physical_H3chi_H4mu_H4delta_norm"], 1e-30), "field": field, "curvature": curvature, "valid_for_physics_claim": False})
        ...
        report["all_hybrid_curvature_sensitivity_gates_passed"] = reconstructions["hybrid"][1]["all_curvature_sensitivity_gates_passed"]
        report["all_hybrid_strong_mesh_differences_reduced"] = all(row["branches"]["hybrid"]["physical_ratio"] >= 1.3 for row in report["refinement"])
