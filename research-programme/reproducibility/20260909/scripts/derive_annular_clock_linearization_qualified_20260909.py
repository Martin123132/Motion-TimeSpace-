...
def run():
    ...
    from annular_clock_linearization_20260909 import clock_field_cross, clock_hessian_matrix, connection_integration, connection_metric_jets
    from annular_clock_complex_control_20260909 import clock_covector
    ...
    destination = intake / "annular-clock-linearized-coupling-qualified"
    ...
    try:
        ...
        legacy_path = intake / "annular-clock-linearized-coupling-derived/status.json"
        own(legacy_path)
        legacy = json.loads(legacy_path.read_text())
        verify("original_complex_kernel_control_failure_preserved", legacy["state"] == "failed" and legacy["passed"] == 231 and legacy["total"] == 232 and not legacy_path.with_name("COMPLETE").exists() and [check["name"] for check in legacy["checks"] if not check["passed"]] == ["nonlinear_modulated_N128_T0.1_clock_coframe_independent_field_clock_block_2"])
        own(root / "scripts/annular_clock_complex_control_20260909.py")
