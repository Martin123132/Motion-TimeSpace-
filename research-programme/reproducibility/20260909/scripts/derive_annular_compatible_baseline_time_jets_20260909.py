...
def run():
    ...
    destination = intake / "annular-compatible-baseline-time-jets"
    ...
    save()
    try:
        folder = intake / "annular-compatible-current-matched-baseline"
        path = folder / "status.json"
        own(path)
        owner = json.loads(path.read_text())
        report["field_evolution_passed"] = owner["state"] == "complete" and owner["passed"] == owner["total"]
        report["retained_field_failures"] = [row for row in owner["checks"] if not row["passed"]]
        verify("field_evolution_terminal_and_reported", owner["state"] in {"complete", "failed"} and "failure" not in owner and len(owner["runs"]) == 16 and owner["passed"] == sum(row["passed"] for row in owner["checks"]) and (folder / "COMPLETE").exists() == report["field_evolution_passed"])
        ...
        for name in ["derive_annular_compatible_baseline_time_jets_20260909.py", "annular_coupled_current_time_jets_20260909.py", "derive_annular_coupled_current_time_jets_20260909.py", "annular_correction_time_jets_20260909.py", "annular_coupled_current_operator_20260909.py", "annular_coordinate_evolution_operator_20260909.py", "annular_evolution_operator_20260909.py", "annular_fifth_order_jet_20260909.py", "annular_noether_completion_20260909.py", "annular_noether_source_projection_20260909.py", "sbp4_compatible_second_operator_20260909.py", "sbp4_derived_operator_20260909.py", "navier_stokes_source_audit_20260908.py"]:
        ...
        for case_name in ["canonical", "nonlinear_modulated"]:
            ...
            for entry in owner["runs"]:
                if entry["case"] != case_name or entry["time_refinement"] != (2 if entry["intervals"] == 128 else 1):
                    continue
