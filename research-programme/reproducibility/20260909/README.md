...
the capsule root. Run a copied script from this directory, for example:

```powershell
Set-Location C:\path\to\20260909
python scripts\derive_annular_spatial_clock_energy_20260909.py
```

The capsule is intended for inspection and rerun preparation, not an automatic
re-execution of the sealed jobs. Install the required compatible Python
runtime packages separately; they are not part of this source closure. The
seal's own results are conditional finite-dimensional evidence. The paired
