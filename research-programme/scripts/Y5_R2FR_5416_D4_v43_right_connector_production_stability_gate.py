...
PREVIOUS = (
    ...
    / "functional_rg"
    / "5415"
    / "v43_left_and_top_path_completion_result.json"
)
...
DOCUMENT = POST / "5416-Y5-R2FR-D4-v43-right-connector-production-stability-gate.md"
SUMMARY = OUTPUT / "v43_right_connector_production_stability_summary.csv"
VALIDATION = OUTPUT / "P8_Y5_BRR5396_5416_VALIDATION.csv"
RESULT = OUTPUT / "v43_right_connector_production_stability_result.json"

REVISION = "D4-deformed-contour-regular-away-W3-v43"
EXPECTED_AREA = 0.03932753620548857
EXPECTED_BASELINE_ACCEPTED = 21
EXPECTED_ACCEPTED = 131
EXPECTED_PENDING = 14
EXPECTED_MAXIMUM_DEPTH = 17
AUDITED_PATH = "LLDLDRDLDLDRDLDRU"
AUDITED_DENOMINATOR = 0.000613556963558325
AUDITED_JACOBIAN = 4.924765244435844
AUDITED_METHOD = "SUBDIVIDED_PATH_CORRELATED_PROJECTIVE_UNION_8X8"
...
def write_document(payload: dict[str, Any]) -> None:
    lines = [
        ...
        f"- certified parameter-area coverage: `{100.0 * payload['accepted_area_fraction']:.15g}%`;",
        f"- accepted plus pending area: `{payload['total_parameter_area']:.17g}`;",
        f"- failure classes: `{payload['failure_category_count']}`, all already declared at checkpoint 5415.",
        "",
        "## Audited transition",
        "",
        f"The depth-`17` path `{payload['audited_refinement_path']}` is committed with amplitude-denominator lower bound `{payload['audited_denominator_abs_lower']:.17g}` and collision-Jacobian lower bound `{payload['audited_collision_jacobian_abs_lower']:.17g}`. Its certificate uses `{payload['audited_collision_jacobian_method']}` rather than a point sample or fitted closure.",
        "",
        "## Claim boundary",
        "",
...
def run_gate() -> dict[str, Any]:
    ...
    required = (PARENT_SCRIPT, PREVIOUS, STATUS, RIGHT_STATE, RIGHT_ROWS)
    ...
    audited_rows = [
        row for row in right_rows if row["refinement_path"] == AUDITED_PATH
    ]
    audited = audited_rows[0] if len(audited_rows) == 1 else None
    ...
    payload: dict[str, Any] = {
        ...
        "baseline_accepted_box_count": int(previous["right_connector_accepted_box_count"]),
        "accepted_box_count": len(right_rows),
        "accepted_box_gain": len(right_rows)
        - int(previous["right_connector_accepted_box_count"]),
        ...
        "audited_refinement_path": AUDITED_PATH,
        "audited_denominator_abs_lower": float(
            audited["minimum_amplitude_denominator_abs_lower"]
        ) if audited else math.nan,
        "audited_collision_jacobian_abs_lower": float(
            audited["collision_jacobian_abs_lower"]
        ) if audited else math.nan,
        "audited_collision_jacobian_method": (
            audited["collision_jacobian_enclosure_method"] if audited else ""
        ),
        "valid_for_committed_depth17_transition": audited is not None,
    ...
    validations = [
        ...
        check(
            "checkpoint_5415_authorizes_v43_only",
            ...
            and not bool(previous["valid_for_full_MTS_claim"]),
            "5415 authorizes continuation while broad claims remain false",
        ),
        check(
        ...
        check(
            ...
            payload["baseline_accepted_box_count"] == EXPECTED_BASELINE_ACCEPTED
            and payload["accepted_box_count"] == EXPECTED_ACCEPTED
            and payload["accepted_box_gain"] == 110
            and payload["completed_path_jobs"] == 29,
            f"accepted {payload['baseline_accepted_box_count']} -> {payload['accepted_box_count']}; jobs=29/240",
        ),
        check(
            "frontier_matches_frozen_run",
            payload["pending_box_count"] == EXPECTED_PENDING
            and payload["maximum_pending_depth"] == EXPECTED_MAXIMUM_DEPTH,
            f"pending={payload['pending_box_count']}; depth={payload['maximum_pending_depth']}",
        ),
        check(
        ...
        check(
            ...
            set(failure_counts) == EXPECTED_RIGHT_FAILURE_CATEGORIES,
            f"category count={len(failure_counts)}",
        ),
        check(
            "audited_depth17_transition_is_committed",
            audited is not None
            and int(audited["refinement_depth"]) == 17
            and math.isclose(
                float(audited["minimum_amplitude_denominator_abs_lower"]),
                AUDITED_DENOMINATOR,
                rel_tol=0.0,
                abs_tol=1.0e-18,
            )
            and math.isclose(
                float(audited["collision_jacobian_abs_lower"]),
                AUDITED_JACOBIAN,
                rel_tol=0.0,
                abs_tol=1.0e-14,
            )
            and audited["collision_jacobian_enclosure_method"] == AUDITED_METHOD,
            AUDITED_PATH,
        ),
        check(
            "all_accepted_rows_are_finite",
            proof_rows_valid(right_rows),
            f"{len(right_rows)} rows checked",
        ),
        check(
            "all_accepted_rows_have_local_certificate_scope",
            row_scope_valid,
            "contour-deformation and regular-away row certificates are true",
        ),
        check(
