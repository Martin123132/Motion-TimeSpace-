...
DOCUMENT = POST / "5463-Y5-R2FR-D4-generalized-connector-finite-projective-amplitude-runner.md"
TARGET_STATUS = OUTPUT / "D4_connector_finite_fallback_status.csv"
SOURCE_REGISTER = OUTPUT / "source_register.csv"
VALIDATION = OUTPUT / "P8_Y5_BRR545_5463_VALIDATION.csv"
RESULT = OUTPUT / "D4_connector_finite_fallback_result.json"

CHECKPOINT = 5463
REVISION = "D4-generalized-connector-finite-projective-amplitude-runner-v1"
...
def target_status(target: dict[str, str]) -> dict[str, Any]:
    ...
    if finite_certified:
        status = "CERTIFIED_FINITE_5463"
        route = "checkpoint_5463_finite"
...
def render_aggregate(payload: dict[str, Any]) -> None:
    lines = [
        "# 5463: Generalized D4 connector finite projective amplitude runner",
        "",
        "## Decision",
        "",
        f"**{payload['decision']}**",
        ...
        "## Construction",
        "",
        "Checkpoint 5463 replaces any stalled all-at-once connector evaluation with the same exact determinant atlas and resume-safe finite `16 x 32` amplitude partition used for TOP. Every cell is independently evaluated by parent v51; no failed adaptive result is converted into evidence.",
        "",
        f"Connector representatives already certified externally: `{payload['external_certified_target_count']}`. Certified by the finite fallback: `{payload['finite_certified_target_count']}`. Finite cover pending: `{payload['cover_only_target_count']}`. First failures: `{payload['first_failure_target_count']}`.",
        "",
        "## Claim boundary",
        "",
        "A completed row is one extreme-representative smoke certificate only. It does not certify all outer leaves, event-local W3, the regulator limit, local GR or full MTS.",
        "",
    ]
...
def refresh_aggregate() -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    finite = [row for row in rows if row["status"] == "CERTIFIED_FINITE_5463"]
    ...
    payload = {
        ...
        "decision": (
            "CONNECTOR_FINITE_FALLBACK_FIRST_FAILURE__DERIVE"
            if failed
            else "CONNECTOR_FINITE_FALLBACK_PARTIAL__RESUME"
        ),
        "target_count": len(rows),
        "external_certified_target_count": len(external),
        "finite_certified_target_count": len(finite),
    ...
    validations = [
        check("all_static_sources_exist", all(path.is_file() for path in (Path(__file__), SCRIPT_5461, SCRIPT_5462, REPRESENTATIVES_5456)), 4),
        check("manifest_has_52_unique_connector_targets", len(rows) == 52 and len({row["target_job_id"] for row in rows}) == 52, len(rows)),
        check("every_finite_certificate_has_a_valid_result", all(Path(row["certificate_path"]).is_file() and truth(read_json(Path(row["result_path"])).get("valid_for_target_outer_representative", False)) for row in finite), len(finite)),
        check("every_materialized_finite_result_preserves_broad_claim_boundary", all(not truth(read_json(path).get("valid_for_full_outer_parent_leaf_enclosure", False)) and not truth(read_json(path).get("valid_for_D4_event_local_W3_bound", False)) and not truth(read_json(path).get("valid_for_all_operator_local_GR_claim", False)) and not truth(read_json(path).get("valid_for_full_MTS_claim", False)) for path in finite_results), len(finite_results)),
    ...
    source_paths = [Path(__file__), SCRIPT_5461, SCRIPT_5462, REPRESENTATIVES_5456]
    source_rows = [
        {
            "checkpoint": CHECKPOINT,
...
def configure_base(target_job_id: str) -> Any:
    helper = load_module("mts_5462_for_5463", SCRIPT_5462)
    ...
    def source_paths() -> list[Path]:
        return list(dict.fromkeys([Path(__file__), SCRIPT_5462, *original_source_paths()]))
    ...
    def amplitude_manifest(
        ...
        for row in rows:
            row["smoke_job_id"] = row["smoke_job_id"].replace("5462__", "5463__", 1)
            row["valid_for_5463_amplitude_manifest"] = row.pop(
    ...
    def representative_certificate(
        ...
        row["certificate_source"] = (
            "checkpoint_5463_connector_finite_projective_amplitude_cover"
        )
        return row

    def render_document(payload: dict[str, Any]) -> None:
        lines = [
            f"# 5463 connector target: {target_job_id}",
            "",
            "## Decision",
            "",
            f"**{payload['decision']}**",
            "",
            "The exact projective atlas and parent-v51 amplitude evaluator are recomputed on this connector's own epsilon/x/t domain using a resume-safe `16 x 32` partition.",
            "",
            ...
            "",
            "This is one outer representative only; all broader claims remain false.",
            "",
        ]
...
def normalize_payload(payload: dict[str, Any], target_job_id: str) -> dict[str, Any]:
    ...
    if truth(payload.get("valid_for_target_outer_representative", False)):
        payload["decision"] = (
            "CONNECTOR_FINITE_PROJECTIVE_AMPLITUDE_COVER_CERTIFIED__RECONCILE_5460"
        )
        payload["next_target"] = "REFRESH_5463_AND_RECONCILE_5460"
    elif payload.get("first_failure_type"):
        payload["decision"] = "CONNECTOR_FINITE_AMPLITUDE_FIRST_FAILURE__DERIVE"
        payload["next_target"] = "DERIVE_FIRST_FAILED_AMPLITUDE_DENOMINATOR"
    elif cover_valid:
        payload["decision"] = (
            "CONNECTOR_PROJECTIVE_COVER_CERTIFIED__AMPLITUDE_RESUME_REQUIRED"
        )
        payload["next_target"] = "RESUME_FINITE_AMPLITUDE_CELLS"
    else:
        payload["decision"] = "CONNECTOR_PROJECTIVE_COVER_NOT_CLOSED"
        payload["next_target"] = "DERIVE_PROJECTIVE_COVER"
    return payload
...
def run_target(
    ...
    if before_formalization != after_formalization:
        raise RuntimeError("formalization-workbench changed during checkpoint 5463")
...
def main() -> int:
    ...
    if not arguments.target_job_id:
        raise ValueError("--target-job-id is required for a finite connector run")
    payload = run_target(
