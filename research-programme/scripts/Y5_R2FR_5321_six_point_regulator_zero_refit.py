...
INPUT_AUDIT = SOURCE / "six_regulator_zero_fit_input_audit.csv"
MODEL_FITS = SOURCE / "six_regulator_zero_model_fits.csv"
STABILITY = SOURCE / "six_regulator_zero_fit_stability.csv"
RICHARDSON = SOURCE / "six_regulator_pairwise_richardson.csv"
SUMMARY = SOURCE / "six_regulator_zero_limit_summary.csv"
RESULT = SOURCE / "six_point_regulator_zero_refit_result.json"
VALIDATION = SOURCE / "six_point_regulator_zero_refit_validation.csv"
RESIDUAL_VALIDATION = RESIDUALS / "P8_Y5_BRR545_5321_VALIDATION.csv"
DOCUMENT = POST / "5321-Y5-R2FR-six-point-regulator-zero-refit.md"

CHECKPOINT = 5321
PARENT_CHECKPOINT = 5320
MARKER = "MTS_5321_SIX_POINT_REGULATOR_ZERO_REFIT"
REVISION = "six-point-regulator-zero-refit-v1"
EXPECTED_IDS = ("E00125", "E0025", "E005", "E010", "E020", "E040")
...
M5319 = load_module("mts_5319_for_5321", SCRIPT_5319)
...
def write_csv(
    ...
    M5319.write_csv(path, rows, leading_fields)


def read_json(path: Path) -> dict[str, Any]:
    return M5319.read_json(path)


def atomic_json(path: Path, value: Any) -> None:
    M5319.atomic_json(path, value)
...
def input_arrays() -> tuple[list[dict[str, Any]], np.ndarray, np.ndarray, np.ndarray]:
    ...
    for row in rows:
        ...
        source_path = FINITE_5320 if row["epsilon_id"] == "E00125" else LADDER_5318
        audited.append(
            {
...
def execute() -> dict[str, Any]:
    ...
    parent = read_json(RESULT_5320)
    parent_validation = read_csv(VALIDATION_5320)
    ...
    input_gate = (
        ...
        and parent["decision"] == "E00125_FINITE_REGULATOR_CONVERGED__REFIT_ZERO_LIMIT"
    ...
    if not input_gate:
        raise RuntimeError("six-point regulator-zero input gate failed")
    ...
    if accepted:
        decision = "SIX_POINT_FIXED_DECAY_ZERO_LIMIT_ACCEPTED__BUILD_DECAY_ANGLE_LADDER"
        ...
        decision = "SIX_POINT_ZERO_LIMIT_UNRESOLVED__EXTEND_SMALL_EPSILON_LADDER"
    ...
    write_csv(
        ...
        [
            {
                "summary_id": "SIX_POINT_FIXED_DECAY_REGULATOR_ZERO",
    ...
    result = {
        ...
        "marker": MARKER,
        "revision": REVISION,
        "mode": "six-point-regulator-zero-normal-form-refit",
        ...
        **complex_fields("reference_zero_limit", reference),
        ...
        "required_next_regulator_id": None if accepted else "E000625",
...
def validate_outputs() -> dict[str, Any]:
    ...
    blocker_consistent = (
        ...
        or (
            result["required_next_regulator_id"] == "E000625"
            and result["decision"]
            in {
                "SIX_POINT_LIMIT_STABLE__ADD_E000625_TO_CLOSE_REMAINDER_ENVELOPE",
                "SIX_POINT_ZERO_LIMIT_UNRESOLVED__EXTEND_SMALL_EPSILON_LADDER",
            }
    ...
    gates = [
        validation_gate(
            "six_valid_finite_regulator_inputs",
            len(inputs) == 6
            and tuple(row["epsilon_id"] for row in inputs) == EXPECTED_IDS
        ...
        validation_gate(
            "six_point_stability_tree_complete",
            bool(stability)
        ...
        validation_gate(
            "five_adjacent_richardson_pairs_complete",
            len(richardson) == 5,
    ...
    return {
        ...
        "decision": (
            "VALIDATED_SIX_POINT_REGULATOR_ZERO_REFIT"
            if passed
            else "SIX_POINT_REGULATOR_ZERO_REFIT_VALIDATION_FAILED"
        ),
        "runtime_seconds": time.perf_counter() - started,
    }
