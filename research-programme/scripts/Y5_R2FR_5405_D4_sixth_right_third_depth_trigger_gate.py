...
OUTPUT = POST / "source-intake" / "functional_rg" / "5405"
PREVIOUS = (
    POST
    / "source-intake"
    / "functional_rg"
    / "5404"
    / "sixth_right_second_depth_trigger_gate_result.json"
)
STATE = (
    SOURCE
    / "partial"
...
AUDITED_PATH = "RDRDLDRDLDLDRU"
SCOPE = "S_X003_MC04_SP_DM_RIGHT_CONNECTOR_d14_RDRDLDRDLDLDRU"
...
def run_gate() -> dict[str, Any]:
    ...
    validations = [
        ...
        check(
            "checkpoint_5404_authorizes_continuation",
        ...
        check(
            ...
            len(rows) == 320 and int(state["accepted_count"]) == len(rows),
        ...
        check(
            ...
            "10 pending boxes; maximum depth 13; audited leaf removed from stack",
        ),
        check(
    ...
    atomic_csv(OUTPUT / "P8_Y5_BRR5396_5405_VALIDATION.csv", validations)
    atomic_json(OUTPUT / "sixth_right_third_depth_trigger_gate_result.json", payload)
    print(json.dumps(payload, indent=2, sort_keys=True))
    if failed:
        raise RuntimeError(
            "checkpoint 5405 validation failed: "
