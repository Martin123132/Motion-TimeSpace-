...
PREVIOUS = (
    ...
    / "functional_rg"
    / "5419"
    / "v43_depth17_sibling_commit_result.json"
)
PREVIOUS_VALIDATION = (
    POST
    / "source-intake"
    / "functional_rg"
    / "5419"
    / "P8_Y5_BRR5396_5419_VALIDATION.csv"
)
...
DOCUMENT = POST / "5420-Y5-R2FR-D4-v43-depth15-to-depth11-frontier-gate.md"
SUMMARY = OUTPUT / "v43_depth15_to_depth11_frontier_summary.csv"
VALIDATION = OUTPUT / "P8_Y5_BRR5396_5420_VALIDATION.csv"
RESULT = OUTPUT / "v43_depth15_to_depth11_frontier_result.json"

REVISION = "D4-deformed-contour-regular-away-W3-v43"
EXPECTED_AREA = 0.03932753620548857
EXPECTED_BASELINE_ACCEPTED = 237
EXPECTED_ACCEPTED = 239
EXPECTED_PENDING = 9
EXPECTED_MAXIMUM_DEPTH = 11
NEW_PATHS = (
    "LLDRDLDLDRDRDRU",
    "LLDRDLDLDRDRU",
)
...
def write_document(payload: dict[str, Any]) -> None:
    lines = [
        "# 5420: v43 depth-15 to depth-11 frontier gate",
        "",
        "## Decision",
        "",
        "**PASS FOR COMMITTED FRONTIER CONTRACTION AND CONTINUED V43 PRODUCTION ONLY.**",
        "",
        "The checkpoint-5419 right-connector frontier advances through the exact depth-15 box and its depth-13 successor under the unchanged production evaluator. No analytic repair, fitted closure, or point-only substitute is introduced.",
        "",
        "## Production evidence",
        "",
        ...
        f"- depth-15 denominator/Jacobian margins: `{payload['depth15_denominator_abs_lower']:.17g}` / `{payload['depth15_collision_jacobian_abs_lower']:.17g}`;",
        f"- depth-13 denominator/Jacobian margins: `{payload['depth13_denominator_abs_lower']:.17g}` / `{payload['depth13_collision_jacobian_abs_lower']:.17g}`;",
        f"- accepted area fraction: `{100.0 * payload['accepted_area_fraction']:.15g}%`.",
        "",
        "The active frontier now begins at depth 11. The four previously known interval-obstruction classes are unchanged.",
        "",
        "## Claim boundary",
        "",
        "The right connector and 211 later contour jobs remain incomplete. Regular-away W3, event-local W3, combined W3, regulator limit, UV, local-GR, and full-MTS claims remain false.",
        "",
    ]
...
def run_gate() -> dict[str, Any]:
    ...
    baseline = int(previous["accepted_box_count"])
    new_rows = rows[baseline:]
    rows_by_path = {row["refinement_path"]: row for row in new_rows}
    ...
    payload: dict[str, Any] = {
        ...
        "baseline_accepted_box_count": baseline,
        "accepted_box_count": len(rows),
        "accepted_box_gain": len(new_rows),
        "baseline_pending_box_count": int(previous["pending_box_count"]),
        "pending_box_count": len(state["stack"]),
        "baseline_maximum_pending_depth": int(previous["maximum_pending_depth"]),
        ...
        "depth15_denominator_abs_lower": float(
            rows_by_path[NEW_PATHS[0]]["minimum_amplitude_denominator_abs_lower"]
        ),
        "depth15_collision_jacobian_abs_lower": float(
            rows_by_path[NEW_PATHS[0]]["collision_jacobian_abs_lower"]
        ),
        "depth13_denominator_abs_lower": float(
            rows_by_path[NEW_PATHS[1]]["minimum_amplitude_denominator_abs_lower"]
        ),
        "depth13_collision_jacobian_abs_lower": float(
            rows_by_path[NEW_PATHS[1]]["collision_jacobian_abs_lower"]
        ),
        "failure_category_count": len(failure_counts),
        "failure_counts": failure_counts,
        "next_refinement_path": str(state["stack"][-1][5]),
    ...
    validations = [
        ...
        check(
            "checkpoint_5419_is_green_and_bounded",
            ...
            "5419 has 14/14 passing gates with broad claims false",
        ),
        check(
        ...
        check(
            "expected_frontier_rows_are_committed",
            baseline == EXPECTED_BASELINE_ACCEPTED
            and len(rows) == EXPECTED_ACCEPTED
            and set(rows_by_path) == set(NEW_PATHS),
            f"accepted {baseline} -> {len(rows)}",
        ),
        check(
            "frontier_contracts_to_depth11",
            len(state["stack"]) == EXPECTED_PENDING
            and maximum_depth == EXPECTED_MAXIMUM_DEPTH,
            f"pending={len(state['stack'])}; depth={maximum_depth}",
        ),
        check(
        ...
        check(
            "failure_classes_are_unchanged",
            set(failure_counts) == set(previous_failures)
            and all(
                failure_counts[key] >= previous_failures[key]
