...
PREVIOUS = (
    ...
    / "functional_rg"
    / "5410"
    / "anisotropic_projective_collision_jacobian_result.json"
)
...
DOCUMENT = POST / "5411-Y5-R2FR-D4-v42-anisotropic-production-stability-gate.md"
SUMMARY = OUTPUT / "v42_anisotropic_production_stability_summary.csv"
VALIDATION = OUTPUT / "P8_Y5_BRR5396_5411_VALIDATION.csv"
RESULT = OUTPUT / "v42_anisotropic_production_stability_result.json"

REVISION = "D4-deformed-contour-regular-away-W3-v42"
TARGET_PATH = "LLDLDLDRDLDRDRDLU"
ANISOTROPIC_METHOD = "SUBDIVIDED_PATH_CORRELATED_PROJECTIVE_UNION_8X16"
SQUARE_METHOD = "SUBDIVIDED_PATH_CORRELATED_PROJECTIVE_UNION_8X8"
...
def source_revision(path: Path) -> tuple[str, bool]:
...
def write_document(payload: dict[str, Any]) -> None:
    lines = [
        "# 5411: v42 anisotropic production-stability gate",
        "",
        "## Decision",
        "",
        "**PASS FOR CONTINUED V42 PRODUCTION ONLY.**",
        "",
        "The anisotropic projective repair is not merely a historical-box patch. Two bounded same-revision resumes retain the existing certificates and advance the active connector without creating a new analytic failure class.",
        "",
        "## Production evidence",
        "",
        f"- accepted boxes advance `{payload['baseline_accepted_box_count']} -> {payload['active_accepted_box_count']}`;",
        f"- the active stack contains `{payload['active_pending_box_count']}` boxes at maximum depth `{payload['active_maximum_pending_depth']}`;",
        f"- exact `8 x 16` projective certificates increase `{payload['baseline_anisotropic_row_count']} -> {payload['anisotropic_production_row_count']}`;",
        f"- the formerly deepest pending path `{TARGET_PATH}` is committed with denominator margin `{payload['target_denominator_abs_lower']:.17g}` and `|J| >= {payload['target_jacobian_abs_lower']:.17g}`;",
        f"- the geometric-denominator split counter remains frozen at `{payload['geometric_failure_count']}`, and the failure-category set remains exactly the five previously declared outer-enclosure classes.",
        "",
        "Accepted plus pending parameter area remains exactly the parent rectangle within outward floating tolerance. No accepted certificate uses a point sample, fitted tolerance, or closure coefficient.",
        "",
        "## Claim boundary",
        "",
        "This checkpoint establishes production stability of the v42 finite-cover proof on the active path only. The path is incomplete, so regular-away W3, event-local W3, combined W3, regulator-limit, UV, local-GR, and full-MTS claims remain open.",
        "",
    ]
...
def run_gate() -> dict[str, Any]:
    ...
    revision, syntax_ok = source_revision(PARENT_SCRIPT)
    failure_counts = {
        str(key): int(value)
    ...
    geometric_count = failure_counts.get(geometric_category, 0)
    ...
    payload: dict[str, Any] = {
        ...
        "square_projective_production_row_count": method_counts.get(
            SQUARE_METHOD, 0
        ),
        "direct_projective_production_row_count": method_counts.get(
            "DIRECT_CENTERED_PROJECTIVE_ALTERNATE", 0
        ),
        "target_refinement_path": TARGET_PATH,
        "target_denominator_abs_lower": (
            float(target_row["minimum_amplitude_denominator_abs_lower"])
            if target_row
            else 0.0
        ),
        "target_jacobian_abs_lower": (
            float(target_row["collision_jacobian_abs_lower"])
            if target_row
            else 0.0
        ),
        "target_jacobian_method": (
            target_row["collision_jacobian_enclosure_method"]
            if target_row
            else "MISSING"
        ),
        ...
        "active_area_coverage_fraction": accepted_area / expected_area,
    ...
    summary_row = {
        ...
        if key not in {"provenance_sha256", "created_utc"}
    }
    atomic_csv(SUMMARY, [summary_row])
    ...
    validations = [
        ...
        check(
            ...
            syntax_ok and revision == REVISION,
            revision or "missing REVISION assignment",
        ),
        check(
            "checkpoint_5410_authorizes_v42_only",
            bool(previous["valid_for_continued_v42_production"])
            and not bool(previous["valid_for_regular_away_W3_claim"]),
            "5410 authorizes production while preserving the broad claim lock",
        ),
        check(
        ...
        check(
            "production_advances_without_revision_change",
            state["revision"] == REVISION
            and len(rows) > int(previous["active_accepted_box_count"]),
            f"accepted {previous['active_accepted_box_count']} -> {len(rows)}",
        ),
        check(
        ...
        check(
            ...
            f"observed category count={len(failure_counts)}",
        ),
        check(
        ...
        check(
            "deepest_preconfirmation_transition_is_committed",
            target_row is not None
            and float(target_row["minimum_amplitude_denominator_abs_lower"]) > 0.0
            and float(target_row["collision_jacobian_abs_lower"]) > 0.0,
            (
                f"path={TARGET_PATH}; denominator="
                f"{payload['target_denominator_abs_lower']:.17g}; J="
                f"{payload['target_jacobian_abs_lower']:.17g}"
            ),
        ),
        check(
            "anisotropic_cover_survives_beyond_5410",
        ...
        check(
            ...
            "regular-away W3, UV, local-GR, and full-MTS remain unclaimed",
        ),
        check(
