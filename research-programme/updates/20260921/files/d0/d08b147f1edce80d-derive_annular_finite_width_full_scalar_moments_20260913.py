...
def run():
    ...
    destination = intake / 'annular-finite-width-full-scalar-attempt02'
    ...
    report['attempt01_validator_correction'] = 'Asymmetric normalized smearing has a nonzero first translation moment. Finite-width displacement is not a failed point limit. Verify the independent Taylor slope and exact zero-width identity, retaining all raw differences; no original C1 gate changes.'
    save()
    try:
        ...
        for gram in [False, True]:
            ...
            for shape in ['beta22', 'beta23']:
                ...
                shift_step = 1 / 4096
                slopes = []
                for shift in [shift_step, shift_step / 2]:
                    plus = TranslatedScalarAction(history, gram, shift).full_integrated(np.ones(4), order=18)
                    minus = TranslatedScalarAction(history, gram, -shift).full_integrated(np.ones(4), order=18)
                    slopes.append({key: (plus[key] - minus[key]) / (2 * shift) for key in ['action', 'raw']})
                slope = {key: (4 * slopes[1][key] - slopes[0][key]) / 3 for key in ['action', 'raw']}
                moment = 0. if shape == 'beta22' else -.1
                zero_width = FiniteWidthScalar(history, gram, 0., shape).integrated(np.ones(4), time_order=18)
                check(branch + '_' + shape + '_exact_zero_width_parent_recovery', max(abs(zero_width[key] - point[key]) for key in ['action', 'raw']) < 1e-12)
                ...
                for trial_width in widths:
                    ...
                    rows.append({'branch': branch, 'shape': shape, 'width': trial_width, 'action_point_error': abs(result['action'] - point['action']), 'variation_point_error': abs(result['raw'] - point['raw']), 'independent_translation_slope_action': slope['action'], 'independent_translation_slope_variation': slope['raw'], 'first_translation_moment': moment, 'action_after_derived_first_moment': abs(result['action'] - point['action'] - trial_width * moment * slope['action']), 'variation_after_derived_first_moment': abs(result['raw'] - point['raw'] - trial_width * moment * slope['raw'])})
                report['point_limit_cases'].extend(rows)
                check(branch + '_' + shape + '_derived_smooth_point_limit_first_moment', max(rows[-1]['action_after_derived_first_moment'], rows[-1]['variation_after_derived_first_moment']) < 1e-9, rows[-1])
                save()
