...
def run():
    ...
    destination = intake / 'annular-interface-layer-clock-attempt01'
        ...
        for source in report['sources']:
            ...
            for name in ['beta22', 'beta33', 'beta23']:
                ...
                for index, relative_width in enumerate(widths):
                    ...
                    row.update({'branch': branch, 'width_index': index})
                    ...
                    family.append(row)
                    save()
                check(branch + '_' + name + '_thin_layer_limits_improve', all(family[-1][key] < family[0][key] / 1000 for key in ['root_limit_error', 'mass_limit_error', 'mean_limit_error', 'G_limit_error', 'P1_integral_limit_error']))
                check(branch + '_' + name + '_shared_clock_rate_not_uniformly_bounded', family[-1]['constant_clock_peak_P1'] > 10000 * family[0]['constant_clock_peak_P1'])
