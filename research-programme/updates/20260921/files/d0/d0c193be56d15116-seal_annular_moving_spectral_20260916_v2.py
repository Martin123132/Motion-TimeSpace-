...
def main():
    ...
    destination=intake/'annular-moving-spectral-final-integrity-v2.json'
    snapshot=intake/'annular-moving-spectral-resume-snapshot-v2.md'
    executed=intake/'annular-moving-spectral-executed-sealer-v2.py'
    ...
    try:
        ...
        failed_seal_path=intake/'annular-moving-spectral-final-integrity.json'
        failed_seal=json.loads(failed_seal_path.read_text())
        check('literal_wording_guard_failure_preserved',failed_seal['state']=='failed' and 'note_complete_scope_and_failure_caveats' in failed_seal['error'])
        inherit(failed_seal)
        own(failed_seal_path)
        failed_attempts.append(dict(folder=failed_seal_path.name,error=failed_seal['error']))
        ...
        check('note_complete_scope_and_failure_caveats','not a full GR limit' in content and 'NOT yet a freely evolved reduced solution' in content
        ...
        modules=['annular_moving_spectral_frame_20260916.py','verify_annular_moving_spectral_connection_20260916.py',
            ...
            'seal_annular_moving_spectral_20260916.py','seal_annular_moving_spectral_20260916_v2.py']
        for name in modules:
