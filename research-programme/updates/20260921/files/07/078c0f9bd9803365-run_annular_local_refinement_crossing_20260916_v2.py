...
def main():
    ...
    parser.add_argument('--base-counts', type=int, nargs='+', default=[257, 513])
    parser.add_argument('--label', required=True)
    arguments = parser.parse_args()
    evidence = EvidenceRun(arguments.label, __file__)
    try:
        intake = evidence.root/'source-intake/navier-stokes/20260914'
        for folder in ['annular-local-refinement-qualification-attempt04', 'annular-boundary-capacity-attempt01']:
            ...
            evidence.own(path)
        decisions = {row['count']: row['source_splits'] for row in status['local_refinement_decisions']}
        previous_folder = intake/'annular-local-refinement-crossing-attempt01'
        previous = json.loads((previous_folder/'status.json').read_text())
        evidence.check('previous_unsafe_trial_and_completed_reference_preserved',previous['state']=='failed' and 'timelike' in previous['error'])
        evidence.own(previous_folder/'status.json')
        spectrum_path = intake/'annular-local-stiffness-attempt01/status.json'
        spectrum_status = json.loads(spectrum_path.read_text())
        evidence.check('frozen_stiffness_measured_before_restart',spectrum_status['state']=='complete' and all(row['passed'] for row in spectrum_status['checks']))
        evidence.own(spectrum_path)
        ...
        exact_forces = np.array([oracle.source_force(unpacked[0],unpacked[1],unpacked[3]) for unpacked in [oracle.unpack(state) for state in exact_states]])
        evidence.report.update(arguments=vars(arguments), original_crossing_data_and_gates_unchanged=True,
            refinement_selected_by_analytic_capacity=True, initial_Ritz_projection_not_adopted=True,
        ...
        for base_count in arguments.base_counts:
            for gram in [False, True]:
                ...
                source_splits = decisions[base_count]
                system = LocallyRefinedSourceAction(base_count, gram, background_mass=0.,source_splits=source_splits)
                previous_case = next((row for row in previous['cases'] if row['base_count']==base_count and row['branch']==branch),None)
                if previous_case is not None:
                    source_path = previous_folder/(branch+'-'+str(base_count)+'.npz')
                    destination = evidence.output/(branch+'-'+str(base_count)+'.npz')
                    destination.write_bytes(source_path.read_bytes())
                    evidence.own(source_path)
                    evidence.own(destination,'outputs')
                    row = dict(previous_case,reused_completed_reference=True)
                    evidence.report['cases'].append(row)
                    evidence.check(branch+str(base_count)+'_finite_base_vertex_crossing',bool(row['original_base_vertices_crossed']))
                    evidence.check(branch+str(base_count)+'_positive_map',row['minimum_jacobian'] > .9)
                    evidence.check(branch+str(base_count)+'_EL_and_energy',row['maximum_euler_residual'] < 2e-10 and row['energy_relative_drift'] < 2e-8)
                    evidence.save()
                    continue
                spectral_history = []
                ...
                for index in range(1, len(times)):
                    step = spectral_step(system,states[-1])
                    if step['eigen_residual'] >= 2e-8:
                        raise RuntimeError('Unresolved stiffness eigenvalue.')
                    spectral_history.append(step)
                    ...
                    solution = solve_ivp(flow,(times[index-1],times[index]),states[-1],t_eval=times[index-1:index+1],
                        method='DOP853',rtol=2e-10,atol=2e-12,max_step=step['maximum_step'],first_step=step['maximum_step']/2)
                    if not solution.success:
                        raise RuntimeError(solution.message)
                    accepted,calls = solution.y.T,solution.nfev
                ...
                errors = [field_comparison(system, state, oracle, exact,order=16) for state, exact in zip(states, exact_states)]
                forces = []
                for instant,state in zip(times,states):
                    coordinates,rates = np.split(state[:-1],2)
                    acceleration = system.acceleration(instant,coordinates,rates)
                    forces.append(float(system.source_mass*acceleration[-1]/(1-rates[-1]**2)**1.5))
                sampled_force_errors = abs(np.array(forces)-exact_forces)
                ...
                row.update(branch=branch, base_count=base_count,source_splits=source_splits,spectral_step_history=spectral_history,
                    forces=forces,sampled_force_errors512=sampled_force_errors.tolist(),
                    stricter_nine_time_absolute_force_gate=bool(max(sampled_force_errors)<2e-7), scalar_dofs=system.count, original_base_vertices_crossed=old_vertices.tolist(),
                ...
                evidence.save()
                print(json.dumps({key: row[key] for key in ['branch','base_count','source_splits','scalar_dofs','maximum_field_error','final_force_absolute_error','strict_force_gate','stricter_nine_time_absolute_force_gate','seconds']}),flush=True)
        evidence.complete()
