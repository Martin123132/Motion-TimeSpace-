...
def main():
    ...
    if not options.tight:
        raise ValueError('This preserved control version requires --tight.')
    label = 'tight-step'
    ...
    try:
        ...
        evidence.report.update(configuration=dict(rtol=rtol, atol=atol, tight=options.tight, final_time=.005),
            ...
            actual_step_refinement=True, reason='Original tighter tolerances left the accepted integration steps unchanged.')
            ...
            for stride in ([1] if options.tight else [1,2]):
                ...
                for interval, (left, right) in enumerate(zip(times[:-1], times[1:])):
                    result = solve_ivp(rhs, (left,right), correction, method='DOP853', rtol=rtol, atol=atol,
                        max_step=min(cap, (right-left)/4), first_step=min(cap, (right-left)/4))
                ...
                row = dict(branch=branch, stride=stride, seconds=time.monotonic()-started, evaluations=evaluations,
                    maximum_step=min(cap,float(np.max(np.diff(times)))/4), sample_count=len(samples), identity_error=identity_error,
