...
def main():
    ...
    parser.add_argument('--maximum-wall-seconds', type=float, default=1500.)
    args = parser.parse_args()
    if not 0 < args.maximum_wall_seconds <= 1500:
        raise ValueError('Wall limit must be positive and at most1500seconds.')
    evidence = EvidenceRun('annular-P2-evolved-source-halving-'+args.branch+'-attempt02', __file__)
    try:
        ...
        evidence.report.update(github_action=False, subagents_used=False, branch=args.branch,
            ...
            duration=4e-5, steps=[16, 32], maximum_wall_seconds=args.maximum_wall_seconds,
        ...
        control_attempt = '02' if args.branch == 'MTS' else '01'
        control_folder = 'annular-P2-indexed-full-pilot-control-'+args.branch+'-attempt'+control_attempt
