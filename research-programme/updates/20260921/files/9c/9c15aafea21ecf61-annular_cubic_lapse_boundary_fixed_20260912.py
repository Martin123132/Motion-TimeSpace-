...
class CubicFamily:
    def __init__(self, context):
        ...
        points, unused_weights = context.quadrature(8)
        values = self.evaluate(points)[0][:, -2:]
        self.primitives = [GlobalPrimitive(self.knots, values[:, column], 8) for column in range(2)]
        self.endpoint_origin = numerical.array([primitive.evaluate(self.basis.radii[:1])[0] for primitive in self.primitives])
        self.endpoint_drift = numerical.array([primitive.evaluate(self.basis.radii[-1:])[0] for primitive in self.primitives]) - self.endpoint_origin
    ...
    def evaluate(self, points):
        ...
        raw, raw_gradient = self.raw(points)
        extra = (raw - hats @ self.projection) @ self.transform
        extra_gradient = (raw_gradient - hat_gradient @ self.projection) @ self.transform
    ...
    def mass(self, points):
        fraction = (points - self.basis.radii[0]) / self.length
        values = numerical.column_stack([primitive.evaluate(points) for primitive in self.primitives]) - self.endpoint_origin - fraction[:, None] * self.endpoint_drift
