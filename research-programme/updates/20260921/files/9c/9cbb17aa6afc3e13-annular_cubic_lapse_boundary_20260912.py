...
class CubicFamily:
    def __init__(self, context):
        ...
        self.endpoint_drift = numerical.array([primitive.evaluate(self.basis.radii[-1:])[0] for primitive in self.primitives])
    ...
    def mass(self, points):
        fraction = (points - self.basis.radii[0]) / self.length
        values = numerical.column_stack([primitive.evaluate(points) for primitive in self.primitives]) - fraction[:, None] * self.endpoint_drift
