...
def run():
    ...
    destination = intake / 'annular-history-ward-attempt02'
    ...
    try:
        ...
        for branch in ['GR', 'metric_Gram']:
            for higher in [False, True]:
                ...
                record = {'branch': branch, 'variant': variant, 'compact_test_count': 17, 'actual_Cddot_max': float(abs(actual).max()), 'transported_constraint_max': float(abs(transported_constraint).max()), 'Euler_source_work_max': float(abs(work).max()), 'full_Ward_prediction_error': float(abs(actual - transported_constraint - work).max()), 'grouped_Euler_work': source_details, 'split_Euler_work': split_details, 'contribution_maxima': {name: float(abs(value).max()) for name, value in contributions.items()}, 'compact_only_not_all_nineteen_boundary_rows': True}
                report['cases'].append(record)
                path = destination / (branch + '_' + variant + '_ward_source_work.npz')
                np.savez_compressed(path, actual=actual, transported_constraint=transported_constraint, Euler_source_work=work, compact_transform=transform, unexplained=actual - transported_constraint - work, **contributions, **grouped, **split)
                own(path, 'outputs')
