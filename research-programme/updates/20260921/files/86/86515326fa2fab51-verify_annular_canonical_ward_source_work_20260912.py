...
def run():
    ...
    enlarged = '--enlarged' in sys.argv[1:]
    source = intake / ('annular-scalar-ward-completion-attempt01' if enlarged else 'annular-acceleration-trace-completion-attempt02')
    destination = intake / ('annular-canonical-ward-enlarged-control-attempt01' if enlarged else 'annular-canonical-ward-control-attempt01')
    ...
    save()
    try:
        prior_path = source / 'status.json' if enlarged else intake / 'annular-second-jet-final-integrity.json'
        ...
        for branch in ['GR', 'metric_Gram']:
            for higher in [False, True]:
                ...
                check(branch + '_' + variant + '_seventeen_independent_compact_tests', np.linalg.matrix_rank(transform) == 17 and abs(model.data['nodes']['eta'][[0, -1]] @ transform).max() < 1e-12)
                prepared = {}
                for surface, values in model.data.items():
                    ...
                    test = values['eta'] @ transform
                    test_r = values['eta_r'] @ transform
                ...
                zeta = .1 * (values['F']**1.5 * local['rates']['P'])[:, None] * ((values['eta'] @ transform) * values['N_r'][:, None] - (values['eta_r'] @ transform) * values['N'][:, None])
                transported_constraint = zeta.T @ (weights * Cbulk)
                if model.gram:
                    node_zeta = .1 * (nodes['F']**1.5 * nodal['rates']['P'])[:, None] * ((nodes['eta'] @ transform) * nodes['N_r'][:, None] - (nodes['eta_r'] @ transform) * nodes['N'][:, None])
                    transported_constraint -= node_zeta.T @ (nodes['R']**2 * np.sqrt(nodes['F']) * model.nodal_density)
                work = sum(contributions.values())
                boundary_symmetry_work = -contributions['mass_P_boundary']
                grouped = {
                    'mass_configuration': contributions['local_mu'] + contributions.get('memory_mu_regular', 0) + contributions.get('memory_mu_nodes', 0),
                    'mass_momentum': contributions['local_P'] + contributions['mass_P_boundary'] + contributions.get('memory_P_regular', 0) + contributions.get('memory_P_nodes', 0),
                    'scalar_configuration': contributions['local_chi'] + contributions['scalar_flux_boundary'] + contributions.get('memory_chi_nodes', 0) + contributions.get('memory_chi_boundary_reaction', 0),
                    'scalar_momentum': contributions['local_pi'],
                }
                split = {}
                for phase in ['mu', 'P', 'pi']:
                    split[phase + '_E1_test0'] = local['gauge'][phase].T @ (weights * local['E_' + phase + '_first'])
                    split[phase + '_E0_test1'] = local['gauge_first'][phase].T @ (weights * local['E_' + phase])
                split['chi_E1_test0'] = -local['gauge']['chi'].T @ (weights * local['second']['pi']) - local['gauge']['chi_r'].T @ (weights * local['flux_first'])
                split['chi_E0_test1'] = -local['gauge_first']['chi'].T @ (weights * local['rates']['pi']) - local['gauge_first']['chi_r'].T @ (weights * local['flux'])
                if model.gram:
                    split['chi_E1_test0'] += nodal['gauge']['chi'].T @ result['gram_scalar_first']
                    split['chi_E0_test1'] += nodal['gauge_first']['chi'].T @ model.first['gram_scalar']
                source_details = {name: float(abs(value).max()) for name, value in grouped.items()}
                split_details = {name: float(abs(value).max()) for name, value in split.items()}
                actual = transform.T @ result['constraint_second']
                record = {'branch': branch, 'variant': variant, 'compact_test_count': 17, 'actual_Cddot_max': float(abs(actual).max()), 'transported_constraint_max': float(abs(transported_constraint).max()), 'Euler_source_work_max': float(abs(work).max()), 'full_Ward_prediction_error': float(abs(actual - transported_constraint - work - boundary_symmetry_work).max()), 'grouped_Euler_work': source_details, 'split_Euler_work': split_details, 'contribution_maxima': {name: float(abs(value).max()) for name, value in contributions.items()}, 'compact_only_not_all_nineteen_boundary_rows': True}
                record['radial_action_boundary_symmetry_work_max'] = float(abs(boundary_symmetry_work).max())
                record['frozen_enlargement_pilot'] = enlarged
                check(branch + '_' + variant + '_full_Ward_balance_with_radial_action_work', record['full_Ward_prediction_error'] < 1e-8, record['full_Ward_prediction_error'])
                report['cases'].append(record)
                path = destination / (branch + '_' + variant + '_ward_source_work.npz')
                np.savez_compressed(path, actual=actual, transported_constraint=transported_constraint, Euler_source_work=work, boundary_symmetry_work=boundary_symmetry_work, compact_transform=transform, unexplained=actual - transported_constraint - work - boundary_symmetry_work, **contributions, **grouped, **split)
                own(path, 'outputs')
                gauge_path = destination / (branch + '_' + variant + '_gauge_test_family.npz')
                gauge_arrays = {surface + '__' + tier + '__' + key: value for surface, item in prepared.items() for tier in ['gauge', 'gauge_first'] for key, value in item[tier].items()}
                np.savez_compressed(gauge_path, compact_transform=transform, **gauge_arrays)
                own(gauge_path, 'outputs')
                save()
