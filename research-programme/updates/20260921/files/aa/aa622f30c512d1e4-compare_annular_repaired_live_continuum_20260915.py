...
def main():
    evidence = EvidenceRun('annular-repaired-live-continuum-comparison-attempt01', __file__)
    try:
        oracle_directory = evidence.output.parent/'annular-live-continuum-refinement-attempt02'
        ...
        oracle_path = oracle_directory/'degree384.npz'
        evidence.own(oracle_path)
        saved_oracle = np.load(oracle_path)
        oracle = LiveContinuumCharacteristics(384, 8, 18, radial_spacing=.025, label_order=12)
