...
def main():
    ...
    prefix = 'annular-candidate-Ward-source-v2'
    ...
    try:
        ...
        failed = inherit(intake/'annular-candidate-Ward-source-final-integrity.json')
        check('failed_checker_broadcast_preserved',failed['state']=='failed' and 'non-broadcastable' in failed['error'])
        run = inherit(intake/'annular-candidate-Ward-source-attempt01/status.json')
        ...
        for summary in run['summaries']:
            ...
            dust = dust*(raw['label_source'][:,None,None]<target)
            ...
            spot = loaded(intake/'annular-candidate-Ward-source-attempt01'/(branch+'-'+extension+'-order'+str(order)+'-spot.npz'))
        ...
        for name in ['annular_candidate_Ward_source_20260921.py','run_annular_candidate_Ward_source_20260921.py',
            'seal_annular_candidate_Ward_source_20260921.py','seal_annular_candidate_Ward_source_v2_20260921.py']:
        ...
        report.update(state='complete',completed_at=datetime.now(timezone.utc).isoformat(),distinct_files_rehashed=len(hashes),
            ...
            implementation_checks=len(run['checks']),total_failed_attempts_preserved=prior['total_failed_attempts_preserved']+1,
