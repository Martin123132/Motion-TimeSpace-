...
def run():
    ...
    from annular_cubic_lapse_final_boundary_20260912 import CubicContext, CubicPreparation, CubicFields, evaluate_cubic_initial
    ...
    report = {'state': 'running', 'started_utc': datetime.now(timezone.utc).isoformat(), 'inputs': {}, 'outputs': {}, 'checks': [], 'cases': [], 'valid_for_physics_claim': False, 'new_evolution': False, 'interval_certificate': False, 'protocol': 'N16 only. Common inner mass Newton tolerance 2e-13 for BOTH branches, below unchanged external 1e-10 full-row/trace gates; 2e-14 strict attempts preserved. Explicit alternative finite quadrature of the analytically integrated gravitational action, with retained boundary and analytically null constant-reference subtraction Uref=sqrt(2/3), not a physical parameter. Full-family nodal momentum-trace completion before kernel mass-flux trace completion. Shared coarse free data and Kseed. Full P1 plus two global cubic lapse directions, old constraints retained plus two new rows. Two derived zero-endpoint mass lifts supply dependent mass coefficients. Natural-gradient corrected and enlarged uncorrected controls use same phase construction and boundary drives.'}
    ...
    try:
        ...
        for name in ['annular_cubic_lapse_final_boundary_20260912.py', Path(__file__).name]:
        ...
        for branch in ['GR', 'metric_Gram']:
            ...
            check(branch + '_integrated_gravity_change_from_original_P1_reported', replay_error < 1e-5, replay_error)
            ...
            for correction in [True]:
                ...
                try:
                    ...
                    archive(label + '_initial_data', {**selected, 'pi_boundary_amplitudes': amplitudes, 'radii': context.basis.radii, 'faces': context.basis.faces, 'lapse_extra_projection': context.family.projection, 'lapse_extra_transform': context.family.transform, 'mass_lift_scales': context.family.mass_scale, 'mass_lift_endpoint_drift': context.family.endpoint_drift, 'mass_lift_endpoint_origin': context.family.endpoint_origin, 'lapse_local_polynomials': context.family.polynomials, 'mass_primitive_prefix': context.family.prefix})
