...
def main():
    ...
    try:
        ...
        folder = intake/'annular-GR-causal-refinement-inputs-attempt01'
        ...
        path = folder/('GR-only-'+str(options.degree)+'-1025.npz')
        restrict_array_reads([path],evidence.output)
        blocked = False
        try:
            with (intake/'annular-joint-finer1025-attempt01/reference1025-8-trajectory.npz').open('rb'):
                pass
        ...
        evidence.report.update(arguments=vars(options),finite_future_trajectories_read=False,
            ...
            retrospective_force_subtraction=False,count=1025,source_splits=8,final_time=.4,
        ...
        for gram in [False,True]:
            ...
            system = LocallyRefinedSourceAction(1025,gram,background_mass=0.,source_splits=8)
            ...
            evidence.own(destination,'outputs')
            row = dict(branch=branch,count=1025,splits=8,degree=options.degree,stride=options.stride,
