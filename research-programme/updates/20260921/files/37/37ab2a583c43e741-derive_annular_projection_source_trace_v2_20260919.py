...
def constant_trace_response(layer, extension, position):
    ...
    left_rows = slice((source_index-1)*order, source_index*order)
    right_rows = slice(source_index*order, (source_index+1)*order)
    left_fraction = (layer.reference_radius[left_rows]-layer.edges[source_index-1])/(layer.edges[source_index]-layer.edges[source_index-1])
    right_fraction = (layer.reference_radius[right_rows]-layer.edges[source_index])/(layer.edges[source_index+1]-layer.edges[source_index])
    missing[left_rows, 0] = left_fraction*(2*left_fraction-1)
    missing[right_rows, 1] = (1-right_fraction)*(1-2*right_fraction)
...
def main():
    evidence = EvidenceRun('annular-projection-source-trace-attempt02', __file__)
    try:
        ...
        failed_path = evidence.output.parent/'annular-projection-source-trace-attempt01/status.json'
        failed = json.loads(failed_path.read_text())
        evidence.own(failed_path)
        evidence.check('nominal_fraction_failure_preserved', failed['state'] == 'failed')
        evidence.report.update(missing_source_basis_evaluated_at_actual_quadrature_coordinates=True)
