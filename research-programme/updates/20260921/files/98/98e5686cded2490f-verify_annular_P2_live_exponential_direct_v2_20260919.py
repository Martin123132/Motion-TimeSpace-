...
def main():
    ...
    parser.add_argument('--reuse-standard', action='store_true')
    args = parser.parse_args()
    evidence = EvidenceRun('annular-P2-live-exponential-direct-'+args.branch+'-attempt02', __file__)
    try:
        ...
        reused = None
        if args.reuse_standard:
            previous_folder = 'annular-P2-live-exponential-direct-'+args.branch+'-attempt01'
            previous_path = evidence.output.parent/previous_folder/'status.json'
            reused = json.loads(previous_path.read_text())
            evidence.own(previous_path)
            evidence.check('reused_completed_control', reused['state'] == 'complete')
        evidence.report.update(RK_control_changed_max_step=True, reused_standard_control=args.reuse_standard)
        endpoints = []
        for tag, tolerance in [('standard', 1e-11), ('tight', 2e-12)]:
            if tag == 'standard' and reused is not None:
                previous_case = next(row for row in reused['cases'] if row['tolerance'] == 'standard')
                name = 'standard-accepted'+str(previous_case['accepted_steps']).zfill(3)+'.npz'
                previous_state = checked_load(evidence, previous_folder, name)
                evidence.check('reused_standard_endpoint', float(previous_state['time']) == duration)
                endpoints.append(previous_state['state'])
                evidence.report['cases'].append(dict(**previous_case, reused=True, new_evaluations=0,
                    maximum_step=duration))
                continue
            maximum_step = duration if tag == 'standard' else duration/2
            first_evaluation = evaluations
            solver = DOP853(rhs, 0., initial.ravel(), duration, first_step=maximum_step,
            ...
            evidence.report['cases'].append(dict(tolerance=tag, rtol=tolerance, atol=tolerance/100,
                evaluations=evaluations-first_evaluation, accepted_steps=accepted, maximum_step=maximum_step))
        ...
        standard = reused['independent_standard_diagnostics'] if reused is not None else diagnostics(system, endpoints[0])
        explicit = diagnostics(system, endpoints[1])
        exponential = reused['exponential_diagnostics'] if reused is not None else diagnostics(system, target['state'])
        ...
        evidence.report.update(relative_energy_RK_tolerance_difference=control_error,
            ...
            force_RK_step_refinement_difference=force_control_error,
        ...
        evidence.check('RK_step_and_tolerance_change_small', control_error < 1e-8, control_error)
