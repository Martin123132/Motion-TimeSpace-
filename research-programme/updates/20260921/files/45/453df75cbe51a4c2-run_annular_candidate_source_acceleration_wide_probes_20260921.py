...
def main():
    ...
    evidence = EvidenceRun('annular-candidate-source-acceleration-wide-probes-attempt01', __file__)
    ...
    try:
        ...
        evidence.report.update(github_action=False, subagents_used=False, original_action_unchanged=True,
            ...
            reference_order=16, material_order=48, derivative_steps=[1e-4,5e-5,2.5e-5], probe_family='wider_parameter_check',
        ...
        for branch, extension in [('reference','reference'),('MTS','primary'),('MTS','alternative')]:
            ...
            for label, state_path in [('endpoint-wide',endpoint_path)]:
                ...
                evidence.save()
        evidence.check('complete_equal_branch_experiment',len(evidence.report['cases']) == 3
            and len(evidence.report['probes']) == 18 and len(evidence.report['samples']) == 192
            and len(evidence.report['derivative_checks']) == 15 and len(evidence.report['tangent_checks']) == 9)
        ...
        evidence.report.update(physical_acceleration_qualified=all(row['passed']
            ...
            evaluations=sum(1 for row in evidence.report['iterations'])+3+18,
