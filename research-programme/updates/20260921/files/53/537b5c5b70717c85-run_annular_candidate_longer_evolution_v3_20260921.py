...
def main():
    ...
    evidence = EvidenceRun('annular-candidate-longer-'+args.horizon+'-attempt03', __file__)
    ...
    try:
        ...
        resume_path = evidence.output.parent/'annular-candidate-longer-1e-5-attempt02/status.json'
        resume = json.loads(resume_path.read_text())
        evidence.own(resume_path)
        evidence.check('failed_source_is_saved_resumable_execution', resume['state'] == 'failed'
            and '40 trials' in resume['error'] and all(row['passed'] for row in resume['checks']))
