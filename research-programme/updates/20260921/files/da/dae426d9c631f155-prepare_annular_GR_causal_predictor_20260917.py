...
def main():
    ...
    try:
        ...
        evidence.report.update(known_benchmark_not_blind_new_experiment=True,finite_future_trajectories_read=False,
            prescribed_flat_background_only=True,original_action_unchanged=True,count=513,source_splits=8,
        ...
        system = LocallyRefinedSourceAction(513,True,background_mass=0.,source_splits=8)
        seed = initial(system)
        for degree in [384,512,768]:
            path = folder/('projection-'+str(degree)+'-513.npz')
            evidence.own(path)
            ...
            destination = evidence.output/('GR-only-'+str(degree)+'-513.npz')
            np.savez_compressed(destination,**data,original_initial=seed)
            evidence.own(destination,'outputs')
            evidence.report['cases'].append(dict(degree=degree,count=513,splits=8,scalar_dofs=system.count,
