...
from annular_decimal_transport_v2_20260919 import DecimalAction, DecimalMap, decimal_array, zero_array, dot, propagate
...
def main():
    evidence = EvidenceRun('annular-decimal-duality-attempt02', __file__)
    ...
    try:
        ...
        failed_path = evidence.output.parent/'annular-decimal-duality-attempt01/status.json'
        failed = json.loads(failed_path.read_text())
        evidence.own(failed_path)
        evidence.check('strict_storage_symmetry_attempt_preserved', failed['state'] == 'failed')
        evidence.report['mass_convention'] = 'Original lower-triangle SPD solve, not unused upper-band entries.'
        fixture(evidence)
        ...
        for branch in ['reference', 'MTS']:
            ...
            for digits, degree in [(32, 48), (48, 64)]:
                with localcontext() as ctx:
                    ...
                    evidence.report.setdefault('mass_storage', []).extend(dict(branch=branch, digits=digits, level=level,
                        unused_upper_triangle_discrepancy=action.unused_upper_triangle_discrepancy, convention=action.mass_convention)
                        for level, action in zip(['coarse', 'fine'], actions))
