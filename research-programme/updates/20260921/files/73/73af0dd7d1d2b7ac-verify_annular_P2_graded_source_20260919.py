...
def main():
    evidence = EvidenceRun('annular-P2-graded-source-algebra-attempt01', __file__)
    try:
        ...
        for count in [65, 129, 257]:
            for gram in [False, True]:
                ...
                row = dict(branch='MTS' if gram else 'reference', base_count=count, scalar_nodes=fine.count,
                    ...
                    lifted_operator_error=float(np.max(abs((fine.lifted @ embedding-coarse.lifted).toarray()))))
