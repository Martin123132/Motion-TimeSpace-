...
def run():
    ...
    from annular_gram_velocity_commutator_20260914 import adaptive_energy_integral
    ...
    try:
        ...
        for path in [Path(__file__),root/'scripts/annular_compatible_h_evolution_20260914.py',root/'scripts/annular_gram_velocity_commutator_20260914.py']:
        ...
        for count in arguments.counts:
            ...
            for gram in [False,True]:
                ...
                initial_quadrature_change=float(abs(coarse-fine).max())
                quadrature_error=initial_quadrature_change
                quadrature_evaluations=48
                if initial_quadrature_change>=1e-9:
                    fine,quadrature_error,quadrature_evaluations=adaptive_energy_integral(system,solution,arguments.duration)
                error=abs(energies[-1,0]-energies[0,0]-fine[0])
                propagation=(np.sqrt(energies[0,0])+fine[1])**2
                check(label+'_integrated_energy_rate_and_quadrature',max(error,quadrature_error)<1e-9,
                      {'balance_error':float(error),'initial_quadrature_change':initial_quadrature_change,'accepted_quadrature_error':quadrature_error,'evaluations':quadrature_evaluations})
