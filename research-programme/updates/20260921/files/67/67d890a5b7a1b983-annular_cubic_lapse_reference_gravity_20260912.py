...
GRAVITY_REFERENCE_ROOT = float(numerical.sqrt(2. / 3.))
...
class CubicPreparation(BasePreparation):
    def mass_equations(self, selected):
        ...
        constraint = self.eta.T @ (weights * ((root_f + 1 / root_f - 2 * GRAVITY_REFERENCE_ROOT) / .2 - root_f * energy))
        constraint += eta_r.T @ (weights * radius * (root_f - GRAVITY_REFERENCE_ROOT) / .1)
        constraint -= self.eta_nodes[-1] * context.basis.radii[-1] * (node_root[-1] - GRAVITY_REFERENCE_ROOT) / .1
        constraint += self.eta_nodes[0] * context.basis.radii[0] * (node_root[0] - GRAVITY_REFERENCE_ROOT) / .1
...
def evaluate_cubic_initial(frame_mass, frame_scalar, data, basis, weights, links, gram, clock, reactions=None, surface='quad', link_surface='links'):
    ...
    root_f, node_root = numerical.sqrt(geometry), numerical.sqrt(nodes['F'])
    constraint = lapse_tests.T @ (weights * ((root_f + 1 / root_f - 2 * GRAVITY_REFERENCE_ROOT) / .2 - root_f * energy))
    constraint += values['eta_r'].T @ (weights * radius * (root_f - GRAVITY_REFERENCE_ROOT) / .1)
    constraint -= data['nodes']['eta'][-1] * basis.radii[-1] * (node_root[-1] - GRAVITY_REFERENCE_ROOT) / .1
    constraint += data['nodes']['eta'][0] * basis.radii[0] * (node_root[0] - GRAVITY_REFERENCE_ROOT) / .1
    if gram:
