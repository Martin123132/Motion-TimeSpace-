...
def main():
    ...
    parser.add_argument('--resume', default='annular-candidate-longer-1e-5-attempt04')
    parser.add_argument('--max-seconds', type=float, default=10000)
    args = parser.parse_args()
    horizon = float(args.horizon)
    evidence = EvidenceRun('annular-candidate-finer-time-attempt'+args.attempt, __file__)
    ...
    try:
        ...
        evidence.report.update(horizon=horizon, horizon_label=args.horizon, controls=False, grids=[2, 4, 8, 16], refinement_triples=[[2, 4, 8], [4, 8, 16]],
        ...
        for branch, extension in [('reference', 'reference'), ('MTS', 'primary'), ('MTS', 'alternative')]:
            ...
            for triple in [[2, 4, 8], [4, 8, 16]]:
                for component, initial_state in [(0, coordinates), (1, momenta)]:
                    for block, section in [('field', slice(None, -1)), ('source', slice(-1, None))]:
                        coarse = float(np.max(abs(difference(endpoints[triple[0]][component][:, section], endpoints[triple[1]][component][:, section]))))
                        fine = float(np.max(abs(difference(endpoints[triple[1]][component][:, section], endpoints[triple[2]][component][:, section]))))
                        signal = float(np.max(abs(difference(endpoints[triple[2]][component][:, section], initial_state[:, section]))))
                        ...
                        evidence.report['refinement'].append(dict(branch=branch, extension=extension, grids=triple, component=component,
                ...
                with localcontext() as context:
                    context.prec = 64
                    first, second, third = [Decimal(energy_rows[count]['energy_drift']) for count in triple]
                    first_difference, second_difference = abs(first-second), abs(second-third)
                floor = float(max(row['paired_diagnostic_resolution'] for count in triple for row in [energy_rows[count]]))
                ...
                evidence.report['energy_refinement'].append(dict(branch=branch, extension=extension, grids=triple,
        ...
        evidence.check('equal_branch_counts', len(evidence.report['cases']) == 90
            and len(evidence.report['endpoints']) == 12
            and len(evidence.report['refinement']) == 24 and len(evidence.report['energy_refinement']) == 6
            and len(evidence.report['quadrature']) == 0
