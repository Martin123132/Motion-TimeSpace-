...
def run():
    ...
    if not args.attempt.isdigit():
        raise ValueError('Attempt must be a decimal label.')
    root = Path(__file__).resolve().parents[1]
    intake = root / 'source-intake/navier-stokes/20260909'
    new_intake = root / 'source-intake/navier-stokes/20260910'
    destination = new_intake / ('annular-paired-variational-energy-derived-attempt' + args.attempt)
    ...
    if args.phase == 'derive':
        ...
        try:
            ...
            for parent in parents:
                ...
                mapped_gradient = numerical.einsum('kij,j->ki', forms['scalar_maps'][1:4], configuration)
                mapped_normalized = numerical.einsum('kij,j->ki', forms['normalized_maps'], velocity)
                mapped_energy = .5 * numerical.sum(forms['weight'] * (mapped_gradient**2 + mapped_normalized**2))
                ...
                close(label + '_derived_graph_equation', higher['graph_time'], higher['expected_graph_time'])
                close(label + '_derived_graph_velocity_equation', higher['graph_velocity_time'], higher['expected_velocity_time'])
