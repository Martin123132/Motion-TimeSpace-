...
def main():
    ...
    parser.add_argument('--horizon', choices=['1e-6', '1e-5'], required=True)
    parser.add_argument('--controls', action='store_true')
    parser.add_argument('--max-seconds', type=float, default=5400)
    args = parser.parse_args()
    horizon = float(args.horizon)
    evidence = EvidenceRun('annular-candidate-longer-'+args.horizon+'-attempt04', __file__)
    started = perf_counter()
    deadline = started+args.max_seconds
    try:
        prior_path = evidence.output.parent/'annular-candidate-endpoint-energy-final-integrity.json'
        prior = json.loads(prior_path.read_text())
        evidence.own(prior_path)
        evidence.check('completed_energy_source', prior['state'] == 'complete' and prior['energy_drift_smoke_passed']
        ...
        evidence.report.update(horizon=horizon, horizon_label=args.horizon, controls=args.controls,
        ...
        resume_path = evidence.output.parent/'annular-candidate-longer-1e-5-attempt03/status.json'
        resume = json.loads(resume_path.read_text())
        evidence.own(resume_path)
        evidence.check('failed_source_is_saved_resumable_execution', resume['state'] == 'failed'
            and 'Safe wall boundary' in resume['error'] and all(row['passed'] for row in resume['checks']))
        ...
        for branch, extension in [('reference', 'reference'), ('MTS', 'primary'), ('MTS', 'alternative')]:
            ...
            for component, initial_state in [(0, coordinates), (1, momenta)]:
                for block, section in [('field', slice(None, -1)), ('source', slice(-1, None))]:
                    ...
                    evidence.report['refinement'].append(dict(branch=branch, extension=extension, component=component,
                        ...
                        ratio=fine/max(coarse, 1e-90), signal_relative_difference=fine/max(signal, 1e-90),
                        diagnostic_floor=floor, differences_resolved=bool(min(coarse, fine) > 4*floor),
                        tolerance=tolerance, passed=bool(fine < tolerance and fine <= .4*coarse+2*floor), valid_for_claim=False))
            with localcontext() as context:
                context.prec = 64
                first, second, third = [Decimal(energy_rows[count]['energy_drift']) for count in [1, 2, 4]]
                first_difference, second_difference = abs(first-second), abs(second-third)
            floor = float(max(row['paired_diagnostic_resolution'] for row in energy_rows.values()))
            ratio = float(second_difference/max(first_difference, Decimal('1e-90')))
            resolved = bool(min(float(first_difference), float(second_difference)) > 4*floor)
            evidence.report['energy_refinement'].append(dict(branch=branch, extension=extension,
                first_difference=str(first_difference), second_difference=str(second_difference), ratio=ratio,
                diagnostic_floor=floor, differences_resolved=resolved, resolved_order=bool(resolved and ratio < .4), valid_for_claim=False))
            if args.controls:
                ...
                finer = trajectory('fine', fine_q, fine_p, fine_rates.copy(), 1, horizon, fine_evaluator, fine_path)
                endpoint_energy('fine', finer, fine_evaluator, 'fine_initial')
                generator = np.random.default_rng(20260921)
                seed = rates+3e-5*generator.standard_normal(rates.shape)
                seed[:, -1] = rates[:, -1]+2e-4*generator.standard_normal(len(rates))
                evidence.check(case+'_all_reverse_seeds_perturbed', np.count_nonzero(seed-endpoints[1][2]) == rates.size)
                seed_path = evidence.output/(case+'-reverse-seed.npz')
                np.savez_compressed(seed_path, seed=seed, forward_midpoint=endpoints[1][2])
                evidence.own(seed_path, 'outputs')
                reverse = trajectory('reverse', endpoints[1][0].copy(), endpoints[1][1].copy(), seed, 1,
                    -horizon, evaluator, endpoints[1][3])
                for component, base, fine_base in [(0, coordinates, fine_q), (1, momenta, fine_p)]:
                    for block, section in [('field', slice(None, -1)), ('source', slice(-1, None))]:
                        increments = difference(endpoints[1][component][:, section], base[:, section])
                        ...
                        reversal = float(np.max(abs(difference(reverse[component][:, section], base[:, section]))))
                        ...
                        evidence.report['reversals'].append(dict(branch=branch, extension=extension, component=component,
                            block=block, error=reversal, tolerance=tolerance, all_components_seeded=True,
                            passed=bool(reversal < tolerance), valid_for_claim=False))
            evidence.save()
        evidence.own(evidence.output/'latest-trial-recovery.npz', 'outputs')
        evidence.check('equal_branch_counts', len(evidence.report['cases']) == (27 if args.controls else 21)
            and len(evidence.report['endpoints']) == (12 if args.controls else 9)
            and len(evidence.report['refinement']) == 12 and len(evidence.report['energy_refinement']) == 3
            and len(evidence.report['quadrature']) == (12 if args.controls else 0)
            and len(evidence.report['reversals']) == (12 if args.controls else 0))
        evidence.report.update(new_coupled_evolution=True,
            ...
            energy_time_order_resolved=all(row['resolved_order'] for row in evidence.report['energy_refinement']), seconds=perf_counter()-started)
