...
from run_annular_live_continuum_evolution_20260915 import evolve
...
def main():
    evidence = EvidenceRun('annular-live-oracle-independent-controls-attempt02', __file__)
    try:
        previous = evidence.output.parent/'annular-live-continuum-evolution-attempt01'
        failed = evidence.output.parent/'annular-live-oracle-independent-controls-attempt01'
        evidence.own(failed/'status.json')
        evidence.own(failed/'static-layer0.npz')
        evidence.check('coarse_variable_change_discretization_failure_preserved', json.loads((failed/'status.json').read_text())['state'] == 'failed')
        live = LiveContinuumCharacteristics(192, 4, 14, radial_spacing=.05, coupling=0.)
        trajectory, initial_geometry, calls = evolve(live, max_step=.0002)
        raw = evidence.output/'canonical-zero-coupling-degree192.npz'
        np.savez_compressed(raw, times=trajectory.t, states=trajectory.y.T)
        evidence.own(raw, 'outputs')
        geometries = [live.solve(state) for state in trajectory.y.T]
        for index in [0, 2, 4]:
            source = initial_geometry.material.source[index]
            static = TwoSidedGRCharacteristics(192)
            ...
            result = solve_ivp(static.rhs, (0., .02), initial, method='DOP853', rtol=2e-11, atol=2e-13,
                               max_step=.0002, first_step=.0002, t_eval=trajectory.t)
            ...
            evidence.own(raw, 'outputs')
            row = dict(layer=index, degree=192, field_error=max(field_errors), source_error=max(source_errors))
            evidence.report['cases'].append(row)
            evidence.save()
            print(row, flush=True)
            evidence.check(str(index)+'_unchanged_independent_variable_change_gate', result.success and max(field_errors) < 2e-8
        ...
        evidence.report.update(zero_coupling_compared_to_different_characteristic_variables=True,
                               no_wave_compared_to_independent_proper_time_ODE=True, original_gates_unchanged=True,
                               coarse_variable_change_failure_retained=True)
        evidence.complete()
