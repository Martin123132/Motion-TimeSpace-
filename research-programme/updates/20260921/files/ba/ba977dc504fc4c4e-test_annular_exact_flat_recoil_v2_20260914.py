...
def run():
    evidence = EvidenceRun('annular-exact-flat-recoil-attempt02', __file__)
    try:
        ...
        failed_path = evidence.output.parent/'annular-exact-flat-recoil-attempt01/status.json'
        failed = json.loads(failed_path.read_text())
        evidence.check('first_time_grid_failure_preserved', failed['state'] == 'failed' and 't_eval' in failed['error'])
        evidence.own(failed_path)
        evidence.report['correction'] = 'Replace floating arange endpoint beyond t_span with closed linspace; deduplicate the terminal event sample. No physics or tolerances changed.'
        reservoir = .003
        for amplitude in [.005, .01, .02]:
            ...
            result = solve_ivp(flat_mirror_rhs(reservoir, amplitude), (0., 6.1), [6., 0., 0., 0.],
                               ...
                               t_eval=np.linspace(0., 6.1, 611), events=pulse_complete)
            ...
            before_event = result.t < final_time-1e-13
            times = np.concatenate([result.t[before_event], [final_time]])
            states = np.vstack([result.y[:, before_event].T, final_state])
