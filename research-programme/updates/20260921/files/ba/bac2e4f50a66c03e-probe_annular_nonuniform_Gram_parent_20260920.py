...
def main():
    evidence = EvidenceRun('annular-nonuniform-Gram-parent-probes-attempt01', __file__)
        ...
        for branch in ['reference', 'MTS']:
            ...
            for level in range(3):
                if level:
                    ...
                    knots = refined
                evidence.check(branch+'_'+str(level)+'_all_native_field_nodes_retained',
                    set(coarse_mesh['nodes']) <= set(knots) and set(fine_mesh['nodes']) <= set(knots))
            ...
            for mesh_label, knots in meshes:
                ...
                evidence.report['meshes'].append(dict(branch=branch, label=mesh_label, count=len(knots),
                    ...
                    all_native_fields_resolved=mesh_label != 'uniform513', valid_for_claim=False))
                for endpoint, trial, test in profiles:
                    ...
                    trial_action = gram_action(coordinate, trial_values, trial_jump, np.longdouble(str(float(anchor))),
                        coefficient, include_gram=branch == 'MTS')
                    test_action = gram_action(coordinate, test_values, test_jump, np.longdouble(str(float(anchor))),
                        coefficient, include_gram=branch == 'MTS')
                    for extension in ['primary', 'nonunique_control']:
                        ...
                        if extension == 'nonunique_control' and branch == 'MTS':
                            ...
                            hinge = np.maximum(coordinate-np.longdouble(str(float(anchor))), 0.)
