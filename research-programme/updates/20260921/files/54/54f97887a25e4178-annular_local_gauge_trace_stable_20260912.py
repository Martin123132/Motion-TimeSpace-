...
def polynomial_completion(model, modes):
    ...
    represented_lift = {surface: values @ lift for surface, values in candidates.items()}
    represented_lift_r = {surface: values @ lift for surface, values in gradients.items()}
    physical_correction_errors = []
    for unused_pass in range(3):
        residual = old['quad']['p'].T @ (weights[:, None] * added['quad'])
        physical_correction_errors.append(float(abs(residual).max()))
        correction = residual / row_scale[:, None]
        for surface in old:
            added[surface] -= represented_lift[surface] @ correction
            added_r[surface] -= represented_lift_r[surface] @ correction
