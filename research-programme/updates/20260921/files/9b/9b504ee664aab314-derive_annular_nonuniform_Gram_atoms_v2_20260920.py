...
from qualify_annular_nonuniform_Gram_precision_20260920 import decimal_fraction, sparse_apply
...
def stable_difference(knots, atoms):
    ...
    for index in range(len(knots)-3):
        ...
        result.append(sum((first[column]*decimal_fraction(weight_first)+second[column]*decimal_fraction(weight_second)
            for column, weight_first, weight_second in responses), Decimal(0))/decimal_fraction(width).sqrt())
    return np.asarray(result, dtype=object)


def stable_sparse(mapping, values):
    return np.asarray(sparse_apply(mapping, values), dtype=object)


def main():
    evidence = EvidenceRun('annular-nonuniform-Gram-atoms-attempt02', __file__)
    ...
    try:
        evidence.report.update(github_action=False, subagents_used=False, no_new_evolution=True,
            ...
            atom_preparation_digits=72, kernel_arithmetic='32-digit local atom sums; exact rational geometry; not full binary64 qualification')
        ...
        for label, knots in sequence:
            ...
            for endpoint, atoms in prepared.items():
                ...
                weights = stable_sparse(sampling, [Decimal.from_float(float(value)) for value in data['coefficient']])
                for extension in ['primary', 'nonunique_control']:
                    factors = [stable_sparse(template, field*(1+np.asarray(distortions)) if extension == 'nonunique_control' else field)
                        for field in fields]
                    bilinear = sum((weight*first*last for weight, first, last in zip(weights, *factors)), Decimal(0))
                    ...
                    error = abs(bilinear-Decimal(reference['bilinear']))
