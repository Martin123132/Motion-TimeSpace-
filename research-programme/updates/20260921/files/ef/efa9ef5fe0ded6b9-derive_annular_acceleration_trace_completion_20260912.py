...
def run():
    ...
    destination = intake / 'annular-acceleration-trace-completion-attempt01'
    ...
    try:
        ...
        for branch in ['GR', 'metric_Gram']:
            ...
            record = {'branch': branch, 'completion': diagnostic, 'first_change_max': first_change, 'first_C_max': float(abs(model.first['constraint']).max()), 'first_Cdot_max': float(abs(model.first['constraint_rate']).max()), 'first_Pdot_trace_max': float(abs(model.first['P_t_nodes'][[0, -1]]).max())}
