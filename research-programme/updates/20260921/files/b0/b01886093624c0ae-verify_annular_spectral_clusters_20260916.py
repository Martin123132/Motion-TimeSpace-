...
def main():
    evidence = EvidenceRun('annular-spectral-clusters-curvature-attempt01',__file__)
    try:
        ...
        for count,splits in [(33,2),(65,4)]:
            for background in [0.,.7]:
                for gram in [False,True]:
                    ...
                    position=6.03
                    frame=curved_system_frame(system,position)
                    errors={}
                    ...
                    for step in [2e-6,1e-6,5e-7]:
                        ...
                        fd.append(dict(step=step,relative_second_frame_error=maximum(approx-frame['vectors_bb'])/max(1.,maximum(frame['vectors_bb']))))
                    evidence.check(label+'_independent_second_frame_control',min(row['relative_second_frame_error'] for row in fd)<3e-3,fd)
                    evidence.report['cases'].append(dict(count=count,source_splits=splits,background_mass=background,
                        ...
                        curvature_analytically_derived=True,finite_difference_not_used_as_curvature=True))
                    evidence.save()
