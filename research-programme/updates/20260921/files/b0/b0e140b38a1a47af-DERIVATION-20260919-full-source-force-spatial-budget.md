# Full source-force spatial budget
...
The four completed new runs have70+59+5+622=756 scoped implementation checks. They are not independent physical validations. The strict transport failure is retained. The first sealer also fails because its broad filename-prefix scan picks up an unrelated, completed September16 linearization run and wrongly expects it to be a new failure. That earlier run is not changed. A versioned sealer uses explicit owned-run paths and preserves the first sealing failure. Two new failed executions are thus retained, increasing the historical count from45 to47. The retained false dual qualification is additional information, not counted as a passed physical check.

The corrected integrity implementation is `scripts/seal_annular_full_force_v2_20260919.py`; its preceding failed manifest is `source-intake/navier-stokes/20260914/annular-full-force-final-integrity.json`. This bookkeeping correction changes no calculation or scientific acceptance threshold.
