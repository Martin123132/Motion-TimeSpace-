...
def main():
    evidence = EvidenceRun('annular-local-refinement-qualification-attempt03',__file__)
    try:
        ...
        for count,splits in [(33,4),(65,8),(129,2),(257,8),(513,4)]:
            for gram in [False,True]:
                ...
                precise_nodes = system.radii.astype(np.longdouble)
                precise_nodes[system.element_indices[:,1]] = (system.edges[:-1].astype(np.longdouble)+system.edges[1:].astype(np.longdouble))/2
                indices,shape,unused = old.features_quadratic(precise_nodes)
                exact_embedding = coo_matrix((shape.ravel(),(np.repeat(np.arange(system.count),3),indices.ravel())),shape=(system.count,old.count)).tocsr()
                embedding = exact_embedding.astype(float)
                ...
                jump_absolute = float(np.max(abs(system.jump.astype(np.longdouble) @ exact_embedding-old.jump.astype(np.longdouble))))
