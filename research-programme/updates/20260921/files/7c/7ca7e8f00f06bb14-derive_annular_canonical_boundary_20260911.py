...
def run():
    ...
    parser.add_argument('--flux-port', choices=['inherited-per-branch', 'paired-GR'], default='inherited-per-branch')
    parser.add_argument('--branches', nargs='+', choices=['GR', 'metric_Gram'], default=['GR', 'metric_Gram'])
    ...
    report['flux_port_policy'] = arguments.flux_port
    ...
    try:
        ...
        for size in arguments.sizes:
            for branch in arguments.branches:
                ...
                if arguments.flux_port == 'paired-GR':
                    common_reference = reference.parent / ('canonical_N' + str(size) + '_GR_sample0.npz')
                    own(common_reference)
                    with numerical.load(common_reference) as common:
                        flux_port = float(common['mass_rate'][0])
                ...
                if branch != 'GR':
                    ...
                    if answer[:count].min() <= .02:
                        answer[:count] = model.lapse_seed
