...
def main():
    evidence = EvidenceRun('annular-repaired-live-continuum-comparison-attempt03', __file__)
    try:
        ...
        failed_directory = evidence.output.parent/'annular-repaired-live-continuum-comparison-attempt02'
        failed_status = json.loads((failed_directory/'status.json').read_text())
        evidence.own(failed_directory/'status.json')
        evidence.check('coarsest_MTS_velocity_failure_retained', failed_status['state'] == 'failed'
                       and failed_status['checks'][-1]['name'] == 'MTS-17_source_and_clock_smoke_accuracy')
        for inherited in failed_status['cases']:
            prior_row = dict(inherited)
            prior_row['inherited_from_failed_comparison'] = True
            prior_row['source_and_clock_smoke_pass'] = bool(prior_row['maximum_source_error'] < 5e-7
                and prior_row['maximum_velocity_error'] < 2e-5 and prior_row['maximum_clock_error'] < 2e-7)
            evidence.own(failed_directory/(prior_row['key']+'-diagnostics.json'))
            evidence.report['cases'].append(prior_row)
        evidence.save()
        for count in [33, 65]:
        ...
        evidence.report.update(independent_live_continuum_comparison_completed=True,
                               coarsest_MTS_source_gate_failed_and_retained=True,
                               same_source_gates_applied_to_finer_grids=True,
                               all_resolutions_source_accuracy_qualified=False,
