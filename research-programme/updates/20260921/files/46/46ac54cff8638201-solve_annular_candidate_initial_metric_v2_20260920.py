...
def main():
    evidence = EvidenceRun('annular-candidate-initial-metric-attempt02', __file__)
    ...
    try:
        evidence.report.update(github_action=False, subagents_used=False, no_new_evolution=True,
            ...
            candidate_initial_metric_qualified=False, initial_physical_time=0.,
            previous_frozen_metric_physical_time=4e-5, same_source_state_as_previous_frozen_pilot=False)
        ...
        for branch in ['reference', 'MTS']:
            saved = checked_load(evidence, 'annular-P2-joint-refinement-257-cap2e-05-attempt01', branch+'-initial.npz')
            ...
            coordinates, original_momenta = saved['coordinates'], saved['momenta']
            ...
            trace = checked_load(evidence, 'annular-moving-duhamel-trajectory-attempt01', branch+'-point032.npz')
            evidence.check(branch+'_coherent_initial_time_and_field', float(trace['physical_time']) == 0.
                and np.array_equal(coordinates[center, :-1], trace['0_position']))
            evidence.check(branch+'_initial_velocity_preserved', float(np.max(abs(rates[center, :-1]
                +trace['0_reverse_velocity']))) < 4e-13)
