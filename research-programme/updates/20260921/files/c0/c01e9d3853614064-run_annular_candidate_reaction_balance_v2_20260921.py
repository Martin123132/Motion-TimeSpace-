...
def main():
    ...
    evidence = EvidenceRun('annular-candidate-reaction-balance-attempt02',__file__)
    ...
    try:
        ...
        failed_path = evidence.output.parent/'annular-candidate-reaction-balance-attempt01/status.json'
        evidence.own(failed_path)
        failed = json.loads(failed_path.read_text())
        evidence.check('first_save_failure_preserved',failed['state'] == 'failed' and 'multiple values' in failed['error'])
        evidence.report['inherited_failed_executions'] += 1
        evidence.report['prior_attempt'] = str(failed_path.relative_to(evidence.root))
            ...
            for label,run in [('initial',main_run),('endpoint-wide',wide_run)]:
                ...
                for order in [16,32]:
                    ...
                    np.savez_compressed(path,labels=labels,weights=weights,cardinal=cardinal,offsets=offsets,
                        ...
                        direct_difference=direct_difference,canonical_force=force,canonical_bulk=evaluated['bulk'],canonical_dust=evaluated['dust'],canonical_gram=evaluated['gram'],
