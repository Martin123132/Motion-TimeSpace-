...
def main():
    evidence = EvidenceRun('annular-moving-spectral-connection-attempt02', __file__)
    try:
            ...
            for background in [0., .7]:
                for gram in [False,True]:
                    ...
                    for position in [6.03,6.035]:
                        ...
                        fd_rows = []
                        accepted_controls = 0
                        for step in [2e-6,1e-6,5e-7,2.5e-7,1.25e-7]:
                            ...
                            accepted_controls = accepted_controls+1 if error<2e-4 else 0
                            if accepted_controls>=2:
                                break
                        evidence.check(label+'_independent_aligned_frame_derivative', accepted_controls>=2, fd_rows)
