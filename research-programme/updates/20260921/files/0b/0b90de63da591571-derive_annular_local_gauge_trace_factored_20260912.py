...
def run():
    ...
    from annular_local_gauge_trace_factored_20260912 import polynomial_completion

    root = Path(__file__).resolve().parents[1]
    intake = root / 'source-intake/navier-stokes/20260912'
    destination = intake / 'annular-local-gauge-trace-factored-attempt01'
    ...
    try:
        ...
        for branch in ['GR', 'metric_Gram']:
            for modes in [2, 4]:
                print('Gauge work/trace-factorized metric polynomial trial: ' + branch + ' modes=' + str(modes), flush=True)
