...
def main():
    ...
    try:
        ...
        for label in options.runs:
            ...
            for case in status['cases']:
                ...
                if old is not None:
                    ...
                    evidence.check(prefix+'_old_full_action_replay',state_replay<2e-8 and force_replay<2e-8,
                        dict(state=state_replay,force=force_replay))
        ...
        if options.control:
            folder = intake/options.control
            status = json.loads((folder/'status.json').read_text())
            evidence.own(folder/'status.json')
            evidence.check('full_duration_tighter_runs_complete',status['state']=='complete'
            ...
            for case in status['cases']:
                ...
                evidence.check(branch+'_full_duration_temporal_control',state_change<2e-8 and force_change<2e-8,
