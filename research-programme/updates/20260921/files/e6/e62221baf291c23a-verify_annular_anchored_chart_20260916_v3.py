...
from annular_anchored_projector_chart_20260916 import jet_pullback, omission_diagnostic, split_frame
from annular_windowed_projector_chart_20260916 import WindowedProjectorChart as AnchoredProjectorChart
...
def main():
    evidence = EvidenceRun('annular-anchored-chart-qualification-attempt03', __file__)
    try:
                ...
                for position in [system.anchor, system.anchor+.0003]:
                    ...
                    for step in [2e-4,1e-4,5e-5,2e-5,1e-5,5e-6]:
                        try:
                            ...
                            if not any(word in str(error) for word in ['lost overlap', 'gap closed']):
                                raise
                            controls.append(dict(step=step, first_error=1., second_error=1., rejected=str(error)))
                            continue
                    ...
                    evidence.report['cases'].append(dict(branch=branch,base_count=base_count,splits=splits,background_mass=background,
                        position=position,retained_count=chart.count,full_count=system.count,derivative_controls=controls,selection_survey=chart.survey,
