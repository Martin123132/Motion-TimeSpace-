...
def peano_split(layer, values):
    ...
    for row in range(count):
        ...
        arithmetic_scale += curvature_scale[first:last] @ abs(integrals)+atom_scale[first:last+1] @ abs(kernel)
...
def main():
    evidence = EvidenceRun('annular-P2-live-weighted-projection-attempt03', __file__)
    try:
        evidence.report.update(github_action=False, subagents_used=False,
            ...
            prior_syntax_and_midpoint_roundoff_budget_failures_preserved=True,
