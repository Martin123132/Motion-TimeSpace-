...
def run():
    ...
    try:
        ...
        def build(size, branch, boundary_velocity):
            ...
            system = ReleasedHermiteRouthian(basis, configuration[:size], configuration[size:], constants, .1, momenta[:size], momenta[size:], source['affine_clock'][0], MetricLinkQuadrature(basis))
        ...
        for branch in ['GR', 'metric_Gram']:
            ...
            for size in arguments.sizes:
                ...
                original_slopes = fine.pi_coeff_seed[size:].copy()
                ...
                seed_state = numerical.concatenate([prolonged_mass[1:], projected_pi[:size], coarse_reactions])
                refined = None
                for protocol in ['original_slopes_control', 'transferred_slopes']:
                    ...
                    if protocol == 'original_slopes_control':
                        fine.pi_coeff_seed[size:] = original_slopes
