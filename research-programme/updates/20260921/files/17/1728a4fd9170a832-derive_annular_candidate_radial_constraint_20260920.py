...
def main():
    evidence = EvidenceRun('annular-candidate-radial-derivation-attempt01', __file__)
    try:
        ...
        evidence.check('full_lapse_variation_mass_constraint', sp.simplify(lapse_euler-(mass_radial-expected_mass)/(coupling*root)) == 0)
        evidence.check('full_mass_variation_lapse_constraint', sp.simplify(mass_euler-lapse/(coupling*root)*(expected_lapse-lapse_radial/lapse)) == 0)
