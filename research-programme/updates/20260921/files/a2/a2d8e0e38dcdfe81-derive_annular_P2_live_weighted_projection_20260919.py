...
def peano_split(layer, values):
    ...
    source_neighbourhood = np.zeros(len(layer.edges), dtype=bool)
    ...
    for row in range(count):
        ...
        singular[row] = atoms[first:last+1] @ kernel
        ...
        bound[row] += np.sum(abs(atoms[first:last+1]*kernel))+abs(boundary[row])
        tolerances[row] = 400*np.finfo(float).eps*(np.sum(abs(coefficients)*abs(values[columns]))
            +abs(layer.lifted_hinge[row]*measured_jump))+1e-23
        if source_rows[row]:
...
def main():
    evidence = EvidenceRun('annular-P2-live-weighted-projection-attempt01', __file__)
    try:
