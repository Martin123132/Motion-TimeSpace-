...
def main():
    evidence = EvidenceRun('annular-full-force-transport-attempt02',__file__)
    ...
    try:
        evidence.report.update(github_action=False,subagents_used=False,no_new_evolution=True,
            ...
            measured_path_defect_not_certified_error_bound=True,strict_dual_gate_required_for_claim=True,
            original_strict_dual_gate_pass=True,maximum_wall_seconds=7200)
        ...
        evidence.check('prior_strict_gate_failure_preserved', failed['state'] == 'failed')
        ...
        for branch in ['reference','MTS']:
            ...
            for name,first,last in [('spatial32',0,2),('spatial64',1,2),('temporal32to64',0,1)]:
                ...
                absolute_products = float(np.sum(abs(dual_fine*phases[last][0]))+np.sum(abs(dual_coarse*phases[first][0])))
                operations = dual_fine.size+dual_coarse.size
                gamma = operations*np.finfo(float).eps/(1-operations*np.finfo(float).eps)
                strict_pass = bool(error <= tolerance)
                evidence.report['original_strict_dual_gate_pass'] &= strict_pass
                evidence.check(branch+'_'+name+'_finite_duality_diagnostic', np.isfinite(error) and np.isfinite(absolute_products))
                evidence.report['duals'].append(dict(branch=branch,comparison=name,forward_pairing=forward_pairing,
                    backward_pairing=backward_pairing,error=error,numerical_tolerance=tolerance,
                    original_strict_gate_pass=strict_pass,absolute_dot_products=absolute_products,
                    dot_roundoff_bound_not_full_exponential_certificate=gamma*absolute_products,valid_for_claim=False))
            results = {}
            for stride in [4,2,1]:
