...
def main():
    evidence = EvidenceRun('annular-candidate-endpoint-energy-attempt03', __file__)
    ...
    try:
        ...
        for branch, extension in [('reference', 'reference'), ('MTS', 'primary'), ('MTS', 'alternative')]:
            ...
            for label, folder, suffix, reference_order, material_order in specifications:
                ...
                recovered_coordinates, recovered_momenta, rates, current, history = endpoint_inverse(evaluator,
                    coordinates, momenta, seed, preconditioner, record)
                final = history[-1]
            ...
            for label in ['one_step', 'two_steps', 'four_steps', 'fine_step']:
                ...
                resolution = float(current['diagnostic_resolution_scale']+baseline['diagnostic_resolution_scale'])
            ...
            floor = float(max(branch_drifts[label]['diagnostic_resolution_scale'] for label in ['one_step', 'two_steps', 'four_steps']))
