...
def main():
    ...
    prefix = 'annular-candidate-finer-time'
    ...
    report = dict(state='running', inputs={}, outputs={}, checks=[], github_action=False, subagents_used=False,
        ...
        independent_refinement_diagnostics=[], quadrature_time_budget=[],
        protected_scan_scope='mtime since2026-09-21T11:06:27Z; not a pre-turn whole-tree hash baseline')
    cache = {}
    ...
    def table(label, rows, source_path, count):
        ...
        for original in rows:
            nested_energy = isinstance(original.get('energy'), dict)
            row = {name:value for name, value in original.items() if not (name == 'energy' and nested_energy)}
            if nested_energy:
                row.update(original['energy'])
    ...
    save()
    try:
        prior = inherit(intake/'annular-candidate-longer-evolution-v2-final-integrity.json')
        check('prior_energy_seal_unchanged', prior['state'] == 'complete' and prior['energy_smoke_passed']
        ...
        selected = {
            'time':intake/'annular-candidate-finer-time-attempt01/status.json',
            'quadrature':intake/'annular-candidate-fine16-quadrature-attempt01/status.json'}
        new_failures = []
        for path in sorted(intake.glob('annular-candidate-finer-time-attempt*/status.json')):
        ...
        for label, run in runs.items():
            ...
            check(label+'_equal_branches', len(run['cases']) == (90 if label == 'time' else 96)
                and len(run['endpoints']) == (12 if label == 'time' else 6)
                and len(run['refinement']) == (24 if label == 'time' else 0)
                and len(run['energy_refinement']) == (6 if label == 'time' else 0)
                and len(run['quadrature']) == (0 if label == 'time' else 12)
                and not run['reversals']
            ...
            for row in run['refinement']:
                ...
                endpoints = [states[(branch, extension, 'forward'+str(count), count)][component][:, section] for count in row['grids']]
                with localcontext() as context:
                    context.prec = 64
                    ...
                    coarse_vector = np.asarray(endpoints[0]-endpoints[1], float)
                    fine_vector = np.asarray(endpoints[1]-endpoints[2], float)
                ...
                check(label+'-'+branch+'-'+extension+'-'+str(component)+'-'+row['block']+'_'+str(row['grids'])+'_independent_refinement',
                    coarse == row['coarse_difference'] and fine == row['fine_difference'] and signal == row['motion_signal']
                    and row['grids'] in [[2, 4, 8], [4, 8, 16]]
                    and row['ratio'] == fine/max(coarse, 1e-90) and row['diagnostic_floor'] == floor
                    and row['differences_resolved'] == (min(coarse, fine) > 4*floor)
                    and row['tolerance'] == tolerance
                    and row['passed'] == (fine < tolerance and fine <= .4*coarse+2*floor))
                resolved = min(coarse, fine) > 4*floor
                peak = np.unravel_index(np.argmax(abs(fine_vector)), fine_vector.shape)
                report['independent_refinement_diagnostics'].append(dict(
                    branch=branch, extension=extension, grids=row['grids'], component=component,
                    block=row['block'], coarse_difference=coarse, fine_difference=fine,
                    motion_signal=signal, ratio=fine/max(coarse, 1e-90),
                    diagnostic_floor=floor, differences_resolved=bool(resolved),
                    observed_order=float(np.log2(coarse/fine)) if resolved and fine > 0 else '',
                    richardson_estimated_remaining_error=fine/3 if resolved else '',
                    richardson_estimate_is_not_a_bound=True,
                    richardson_vector_defect=float(np.max(abs(coarse_vector-4*fine_vector)))/max(coarse+4*fine, 1e-90),
                    fine_error_peak_material_node=int(peak[0]), fine_error_peak_component=int(peak[1]),
                    passed=row['passed'], valid_for_claim=False))
            for row in run['energy_refinement']:
                with localcontext() as context:
                    context.prec = 64
                    values = [Decimal(energies[(row['branch'], row['extension'], 'forward'+str(count))]['energy_drift']) for count in row['grids']]
                    first, second = abs(values[0]-values[1]), abs(values[1]-values[2])
                floor = max(energies[(row['branch'], row['extension'], 'forward'+str(count))]['paired_diagnostic_resolution'] for count in row['grids'])
                ratio = float(second/max(first, Decimal('1e-90')))
                resolved = min(float(first), float(second)) > 4*floor
                check(label+'-'+row['branch']+'-'+row['extension']+'_'+str(row['grids'])+'_energy_refinement', str(first) == row['first_difference']
                ...
                for row in run[key]:
                    ...
                    with localcontext() as context:
                        ...
                        if key == 'reversals':
                            ...
                            fine_state = states[(branch, extension, 'fine', 16)][component][:, section]
                            coarse_state = states[(branch, extension, 'forward16', 16)][component][:, section]
            ...
            if run['controls']:
                check(label+'_matched_sixteen_step_control', run['matched_quadrature_steps'] == 16
                    and not run['reverse_control_repeated']
                    and all(row['step'] == run['horizon']/16 for row in run['cases']))
            check(label+'_summary_flags_match_rows', run['trajectories_qualified'] ==
                ...
                and run['energy_time_order_resolved'] ==
                (bool(run['energy_refinement']) and all(row['resolved_order'] for row in run['energy_refinement'])))
        ...
        finest = [row for row in runs['time']['refinement'] if row['grids'] == [4, 8, 16]]
        check('finest_grid_all_branch_blocks_present', len(finest) == 12 and
            len({(row['branch'], row['extension'], row['component'], row['block']) for row in finest}) == 12)
        report['finest_time_grid_qualified'] = all(row['passed'] for row in finest)
        report['coarser_time_failed_rows'] = [row for row in runs['time']['refinement']
            if row['grids'] == [2, 4, 8] and not row['passed']]
        for row in runs['quadrature']['quadrature']:
            matched = next(candidate for candidate in finest if all(candidate[key] == row[key]
                for key in ['branch', 'extension', 'component', 'block']))
            report['quadrature_time_budget'].append(dict(
                branch=row['branch'], extension=row['extension'], component=row['component'], block=row['block'],
                last_time_difference=matched['fine_difference'], time_difference_resolved=matched['differences_resolved'],
                quadrature_increment_difference=row['increment_difference'],
                quadrature_to_time_difference=row['increment_difference']/matched['fine_difference']
                    if matched['fine_difference'] > 0 else '',
                scale_comparison_only_not_error_bound=True, valid_for_claim=False))
        report['matched_quadrature_qualified'] = all(row['passed'] for row in runs['quadrature']['quadrature'])
        check('unique_steps_and_endpoint_states',
            len({row['case'] for row in report['independent_step_checks']}) == 186
            and len({row['case'] for row in report['independent_energy_checks']}) == 18)
        report['distinct_saved_step_states'] = len({row['path'] for run in runs.values() for row in run['cases']})
        report['distinct_saved_endpoint_energies'] = len({row['path'] for run in runs.values() for row in run['endpoints']})
        check('reused_records_not_miscounted_as_new_states', report['distinct_saved_step_states'] == 138
            and report['distinct_saved_endpoint_energies'] == 15)
        counts['independent'] = [table('state-updates', report['independent_step_checks'], destination, 186),
            table('energy-checks', report['independent_energy_checks'], destination, 18),
            table('refinement-diagnostics', report['independent_refinement_diagnostics'], destination, 24),
            table('quadrature-time-budget', report['quadrature_time_budget'], destination, 12)]
        note = root/'DERIVATION-20260921-eight-sixteen-step-time-refinement.md'
        ...
        for name in cited:
            if (root/name).resolve() != destination.resolve():
                own(root/name)
        own(note)
        for name in ['run_annular_candidate_finer_time_20260921.py',
                'run_annular_candidate_fine16_quadrature_20260921.py', 'seal_annular_candidate_finer_time_20260921.py']:
            path = root/'scripts'/name
            compile(path.read_bytes(), str(path), 'exec')
            own(path)
        check('scripts_compile_no_bytecode', not (root/'scripts/__pycache__').exists())
        protected = root.parent/'formalization-workbench'
        start = datetime(2026, 9, 21, 11, 6, 27, tzinfo=timezone.utc).timestamp()
        ...
        report.update(state='complete', completed_at=datetime.now(timezone.utc).isoformat(), new_coupled_evolution=True,
            ...
            next_target='Recover physical source acceleration from the complete live P_v inverse and P_q v term, include total proper-clock response, then compare the same observable across reference/MTS and spatial resolutions.')
        save()
