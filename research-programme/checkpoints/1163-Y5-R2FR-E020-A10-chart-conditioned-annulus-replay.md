# 5147 E020/A10 chart-conditioned-annulus replay

## Result

The E020 row retains its 5142 row-local KLT proof. All four charts pass their numeric Laurent-order checks, so the 5142 row-local simple-pole certificate remains a verified backstop but is not invoked.
...
`[True, True]`,
cross-node relative difference `7.386270871118726e-12`
and selected corrected value `{'real': 2.7045874510128503, 'imaginary': -0.9217389861269736}`. Current schedule
counts are `{'completed_converged': 52, 'completed_unconverged': 0, 'failed': 0, 'missing': 508}` and the next locked row is
`E020__S512503_N0000__A00__primary24`.
...
Validation failures: `[]`. No GitHub action occurred.
