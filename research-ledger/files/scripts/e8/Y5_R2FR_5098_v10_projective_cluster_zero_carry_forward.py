...
SOURCE = POST / "source-intake" / "functional_rg" / "5098"
RESULT_JSON = SOURCE / "v10_projective_cluster_zero_carry_forward_result.json"
MANIFEST_JSON = SOURCE / "v10_projective_cluster_zero_carry_forward_manifest.json"
VALIDATION_CSV = (
    POST
    / "source-intake"
    / "mts_residuals"
    / "P8_Y5_BRR545_5098_VALIDATION.csv"
)
MARKER = "MTS_5098_V10_PROJECTIVE_CLUSTER_ZERO_CARRY_FORWARD"
REVISION = "job-scoped-projective-cluster-zero-carry-forward-v1"
FORMAL_BASELINE = "b0f45c104b5d1ab4762e3f96f23c6e2b2a7afec8c70612879bda90c90308a758"
SOURCE_DIGEST = "adf27b2fe3dde8a8e93ff80de6802d4be2fdcbfbab1f1b8539327629b398e30d"
TARGET_DIGEST = "52c53e634cec9c39b2e82440b59f6bbe768f8f9883bd368423958d0997d5dc59"
TARGET_JOB = "E040__S507622_N0000__A00__coarse12"
EXPECTED_CARRIED_JOBS = 345
...
def transformed_record(
    ...
    value["5098_carry_forward"] = {
        ...
        "reason": (
            f"5097 is exactly scoped to {TARGET_JOB}; this converged record is "
            "outside the repaired row"
        ),
...
def main() -> None:
    ...
    if missing:
        raise FileNotFoundError(f"missing 5098 inputs: {missing}")
    if (TARGET_RUN / "jobs").exists() and any((TARGET_RUN / "jobs").glob("*.json")):
        raise RuntimeError("5098 target already contains job records")
    module_5079 = load_module("mts_5079_for_5098", SCRIPT_5079)
    ...
    target_config = module_5079.M5077.make_config(
        manifest, "bounded_central_anchor_pilot_v10"
    )
    ...
    contract_delta_valid = bool(
        ...
        and target_config["projective_cross_source_cluster_zero_policy"]["job_scopes"]
        == [TARGET_JOB]
        and activation["pilot_execution_authorized"]
    )
    if not contract_delta_valid:
        raise RuntimeError(f"5098 contract delta rejected: {changed_fields}")
    ...
    if len(source_jobs) != EXPECTED_CARRIED_JOBS or any(
        row["job_key"] == TARGET_JOB for _, row in source_jobs
    ):
        raise RuntimeError("5098 source converged set is not the expected unaffected set")
    ...
    kernel_manifest: list[dict[str, Any]] = []
    job_manifest: list[dict[str, Any]] = []
    for source_job_path, source_job in source_jobs:
        source_topology = SOURCE_RUN / "topologies" / Path(source_job["topology_file"]).name
    ...
    status = {
        ...
        "run_id": "bounded_central_anchor_pilot_v10",
    ...
    checks = [
        ("source_paths_exist", not missing, "all 5098 sources exist"),
        ("contract_delta_exact", contract_delta_valid, str(changed_fields)),
        (
            "source_failure_is_exact_target",
            source_status["blocking_job"]["job_key"] == TARGET_JOB,
            source_status["blocking_job"]["job_key"],
        ),
        (
            "carried_jobs_complete",
            len(target_jobs) == EXPECTED_CARRIED_JOBS,
            str(len(target_jobs)),
        ),
        (
            "target_job_not_carried",
            not (TARGET_RUN / "jobs" / f"{TARGET_JOB}.json").exists(),
            TARGET_JOB,
        ),
        (
            "record_digests_rewritten",
            target_records_valid,
            f"jobs={len(target_jobs)} kernels={len(target_kernels)} topologies={len(target_topologies)}",
        ),
        (
            "activation_authorized",
            activation["pilot_execution_authorized"],
            str(activation["pilot_execution_authorized"]),
        ),
        (
            "status_counts",
            status["completed_converged"] + status["missing"] == 360,
            str(status),
        ),
    ...
    with VALIDATION_CSV.open("w", newline="", encoding="utf-8") as handle:
        ...
        for index, (name, passed, detail) in enumerate(checks, start=1):
            writer.writerow(
                {
                    "check_id": f"V5098_{index:02d}_{name}",
                    "passed": passed,
                    "detail": detail,
                    "checkpoint_marker": MARKER,
                }
    ...
    if failed:
        raise RuntimeError(f"checkpoint 5098 validation failed: {failed}")
