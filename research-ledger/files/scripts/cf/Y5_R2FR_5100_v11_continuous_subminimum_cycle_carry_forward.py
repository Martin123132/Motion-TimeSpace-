...
SOURCE_RUN = POST / "source-intake" / "functional_rg" / "5079" / "runs" / "bounded_central_anchor_pilot_v10"
TARGET_RUN = POST / "source-intake" / "functional_rg" / "5079" / "runs" / "bounded_central_anchor_pilot_v11"
SOURCE = POST / "source-intake" / "functional_rg" / "5100"
RESULT_JSON = SOURCE / "v11_continuous_subminimum_cycle_carry_forward_result.json"
MANIFEST_JSON = SOURCE / "v11_continuous_subminimum_cycle_carry_forward_manifest.json"
VALIDATION_CSV = POST / "source-intake" / "mts_residuals" / "P8_Y5_BRR545_5100_VALIDATION.csv"
MARKER = "MTS_5100_V11_CONTINUOUS_SUBMINIMUM_CYCLE_CARRY_FORWARD"
REVISION = "job-scoped-continuous-subminimum-cycle-carry-forward-v1"
FORMAL_BASELINE = "b0f45c104b5d1ab4762e3f96f23c6e2b2a7afec8c70612879bda90c90308a758"
SOURCE_DIGEST = "52c53e634cec9c39b2e82440b59f6bbe768f8f9883bd368423958d0997d5dc59"
TARGET_DIGEST = "31f1640f4c0927462f69ec2e28b909b9942b485e5d3705b800eb9a5d00b7b0e2"
TARGET_JOB = "E040__S507622_N0000__A10__coarse12"
EXPECTED_CARRIED_JOBS = 355
...
def transform(source: Path, target: Path, replacements: dict[str, str] | None = None) -> tuple[str, str]:
...
def main() -> None:
    ...
    if (TARGET_RUN / "jobs").exists() and any((TARGET_RUN / "jobs").glob("*.json")):
        raise RuntimeError("5100 target already contains jobs")
    module = load_module("mts_5079_for_5100", SCRIPT_5079)
    ...
    target_config = module.M5077.make_config(manifest, "bounded_central_anchor_pilot_v11")
    ...
    contract_valid = bool(
        ...
        and target_config["continuous_subminimum_global_cycle_policy"]["job_scopes"] == [TARGET_JOB]
        and activation["pilot_execution_authorized"]
    )
    if not contract_valid:
        raise RuntimeError(f"5100 contract rejected: {changed_fields}")
    source_jobs = []
    for path in sorted((SOURCE_RUN / "jobs").glob("*.json")):
        row = json.loads(path.read_text(encoding="utf-8"))
        if row.get("status") == "COMPLETED_CONVERGED":
            source_jobs.append((path, row))
    if len(source_jobs) != EXPECTED_CARRIED_JOBS or any(row["job_key"] == TARGET_JOB for _, row in source_jobs):
        raise RuntimeError("5100 converged source set changed")
    ...
    for source_job_path, source_job in source_jobs:
        ...
        hashes = transform(source_kernel, target_kernel, {"topology_file": str(target_topology)})
        aggregate.update("".join(hashes).encode("ascii"))
        target_job = TARGET_RUN / "jobs" / source_job_path.name
        hashes = transform(
            source_job_path,
            target_job,
            {"topology_file": str(target_topology), "kernel_file": str(target_kernel)},
        )
        aggregate.update("".join(hashes).encode("ascii"))
    status = {
        "checkpoint_marker": module.MARKER,
        "revision": module.REVISION,
        "run_id": "bounded_central_anchor_pilot_v11",
    ...
    checks = [
        ("sources_exist", not missing, "all 5100 sources exist"),
    ...
    with VALIDATION_CSV.open("w", newline="", encoding="utf-8") as handle:
        ...
        for index, (name, passed, detail) in enumerate(checks, start=1):
            writer.writerow({"check_id": f"V5100_{index:02d}_{name}", "passed": passed, "detail": detail, "checkpoint_marker": MARKER})
    failed = [name for name, passed, _ in checks if not passed]
    if failed:
        raise RuntimeError(f"checkpoint 5100 validation failed: {failed}")
