...
Version 8  —  post-verification draft
Abstract
...
The best empirical scalar organiser of the residual mode amplitude is log(Rb/δu) (functional leave-one-out RMSE ≈ 0.004–0.005 on the shared window, confirmed to be better than the pair (Rb, δu) which gives LOO ≈ 0.022).
Note: The absolute LOO RMSE values are sensitive to the functional regression implementation. The ranking log(Rb/δu) > Rb/δu pair is confirmed from raw files. Any claim about exact LOO magnitudes from chunks 3–4 should be treated with caution pending a definitive implementation.
...
The leading boundary-approach signal in the LTG buffered population is outer gas fraction: Pearson(f_gas,out, δu) = +0.447, Pearson(f_gas,out, x_cross) = −0.382 (both verified from raw files, n=77). Gas-richer buffered LTGs sit farther from the threshold and cross earlier. Near-threshold buffered LTGs (x_cross > 0.75) have lower median outer gas fraction (0.057) than the rest (0.082).
No comparable signal from Rb, Rd, or outer disk surface brightness in the LTG buffered sample.
...
The S → Umax law confirmed for LTGs (N=141, verified from raw files):
...
Level 10 — Joint (S, u_out) population map; LTG outer route split  [new in v8]
...
q ≈ 0.77 global; Rmax = 1.758948; C = 0.459411; U* = 0.201803 (CV = 0.00% by construction); Aloc = 3.344 (CV = 2.56%, 3-galaxy calibration); Stage-4 exponents a ≈ 0.78, b ≈ 1.61 (full sample); buffered profile collapse coefficients u(x) ≈ 1 − 1.588(x−1) + 1.392(x−1)² on shared window [0.92, 1.04]; outer bulge Keplerian tail log-slope = −0.500 for all buffered ETGs; Pearson(f_gas,out, δu) = +0.447 for buffered LTGs.
Not locked / open
Absolute amplitude convention for residual mode: factor-of-2.02 discrepancy between stated convention and computed table unresolved. Functional LOO values from chunks 3–4 of the workbench have an implementation inconsistency; the ranking log(Rb/δu) > pair is confirmed but absolute magnitudes should not be cited.
...
D. Independent second two-channel H+ galaxy to confirm NGC4203 solution.
...
G. What physical variable sets the residual-mode steepness amplitude (best current candidate: log(Rb/δu), unconfirmed).
H. IllustrisTNG/EAGLE test.
I. What explains the remaining ∼60–65% of Q12 variance not captured by fbul alone.
J. Stabilise LTG N discrepancy (my N=141 vs doc N=153).
18.  Results Summary
...
New results in v8: (1) The exact outer control variable δu = 1 − u = (Xcrit − Xout)/Xcrit is confirmed for all 16 ETGs. (2) Buffered outer-viable ETGs share a common rescaled radial approach profile near u = 1, with a one-mode residual structure. (3) The joint (S, u_out) plane maps 175 SPARC galaxies and confirms the ETG-exclusive empty quadrant S ≥ 1.7, u_out < 1. (4) LTGs show the same low-load / buffered outer route split (81/77), with gas fraction as the leading boundary-approach signal. (5) The S → Umax law is confirmed in LTGs (R² = 0.923, N=141), establishing it as a general cross-type result.
20.  Methods and Data
